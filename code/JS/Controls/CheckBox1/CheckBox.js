...
Agi.Controls.CheckBox = Agi.OOP.Class.Create(Agi.Controls.ControlBasic, {
    ...
    RemoveEntity: function (_EntityKey) {
        this.Set("CheckBoxTextField", null);
        this.Set("CheckBoxValueField", null);
...
//            newCheckBoxPositionpars = null;
            ...
            var nId = "CheckBox" + Agi.Script.CreateControlGUID();
            conf.ControlID = nId;
            conf.HTMLElement = nId;
            //conf.ControlBaseObj = nId;
            var NewCheckBox = new Agi.Controls.CheckBox();
            NewCheckBox.CreateControl(conf, Agi.Edit.workspace.canvas);
            return NewCheckBox;
        }
    },
    ...
    EnterEditState: function () {
        ...
        //20130717 倪飘 解决bug，单选按钮1，修改最大显示条数为10后，属性编辑页面，实体标签被挡住（如图）
        obj.css({ "width": "252px", "height": "200px", "overflow": "auto" });
    ...
    ControlAttributeChangeEvent: function (_CheckBoxobj, Key, _Value) {
        ...
        function BindDataByJson(et, d) {
            ...
            var textField = self.Get("CheckBoxTextField") ? self.Get("CheckBoxTextField") : columns[0];
            var valueField = self.Get("CheckBoxValueField ") ? self.Get("CheckBoxValueField ") : columns[0];
...
Agi.Controls.CheckBoxProrityInit = function (_CheckBoxControl) {
    ...
    basicHTML.append("</tr>");
    basicHTML.append("<tr>");
    basicHTML.append("<td class='prortityPanelTabletd0'>列宽:</td><td class='prortityPanelTabletd1'><input id='ColumnWidth' type='number' value='" + CheckBoxBasicProperty.ColumnWidth + "'class='ControlProNumberSty' min='50' max='800'/></td>");
    ...
    basicHTML.append("</tr>");
	basicHTML.append("<tr>");
    basicHTML.append("<td class='prortityPanelTabletd0'>字体大小:</td><td class='prortityPanelTabletd1' colspan='3'><input id='FontSize' type='number' value='" + CheckBoxBasicProperty.FontSize + "'class='ControlProNumberSty' min='8' max='20'/></td>");
    basicHTML.append("</tr>");
    basicHTML.append("<tr>");
    ...
    $(".prortityBindTable").find('#selectText').append($(textOptions)).bind('change', { sels: $(".prortityBindTable").find('#selectText') }, function (e) {
        $(e.data.sels).each(function (i, sel) {
            ...
            _CheckBoxControl.Set("CheckBoxTextField", CheckBoxBasicProperty.CheckBoxTextField);
            $(sel).val(CheckBoxBasicProperty.CheckBoxTextField);
            if (!entity[0].IsShareEntity) {
                ...
                $(entity[0].Data).each(function (i, dd) {
                    CheckBoxBasicProperty.CheckBoxText[i] = dd[CheckBoxBasicProperty.CheckBoxTextField];
    ...
    $(".prortityBindTable").find('#selectValue').append($(valueOptions)).bind('change', { sels: $(".prortityBindTable").find('#selectValue') }, function (e) {
        $(e.data.sels).each(function (i, sel) {
            ...
            _CheckBoxControl.Set("CheckBoxValueField", CheckBoxBasicProperty.CheckBoxValueField);
	...
	$("#FontSize").val(CheckBoxBasicProperty.FontSize);
    $("#FontSize").change(function () {
        var size = parseInt($("#FontSize").val());

        //20130402 倪飘 解决bug，单选按钮列宽输入框中点击向下箭头减小宽度到100，输入值宽为1，弹出信息提示并确定以后，输入框中值变为250但是控件中宽度为10（单选按钮行高也存在相同问题）
        if (size >= 8 && size <= 20) {
            CheckBoxBasicProperty.FontSize = size;
            _CheckBoxControl.Set('CheckBoxBasicProperty', CheckBoxBasicProperty);
        } else {
            $(this).val(CheckBoxBasicProperty.FontSize);
            var DilogboxTitle = "请输入" + parseInt($(this).attr("min")) + "-" + parseInt($(this).attr("max")) + "范围内的值！";
            AgiCommonDialogBox.Alert(DilogboxTitle);
        }
    ...
    $("#UserDefined").bind('click touchstart', function () {
        if (CheckBoxBasicProperty.DataType == "Entity") {
            ...
            if ($("#textFieldAdded").val().trim() == "") {
                AgiCommonDialogBox.Alert("请输入显示值！");
            } else if ($("#valueFieldAdded").val().trim() == "") {
    ...
    $('#SaveChangeButton').bind('click touchstart', function () {
        if ($("#textFieldChanged").val().trim() == "") {
            AgiCommonDialogBox.Alert("请输入显示值！");
        } else if ($("#valueFieldChanged").val().trim() == "") {
            AgiCommonDialogBox.Alert("请输入选择值！");
