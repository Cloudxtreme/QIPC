...
Agi.Controls.CustomCChart = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
        ...
        ChartLoadByData:function(_Data){
            ...
            var jsonData = { "action": "RCalp_Clot",
            ...
            if(_Data!=null && _Data.length>0){
                ...
                Agi.DAL.ReadData({"MethodName":"RCalp_Clot","Paras": jsonString, "CallBackFunction": function (_result) {
                    if (_result && _result.result == "true" && _result.groupdata!=null && _result.groupdata.length>0) {
                        ...
                        for (var i = 0; i < _result.groupdata.length; i++) {
                            ...
                            checkDataSeries.push({
                                ...
                                min: min,
                                max: max,
                                minValue: _result.groupdata[i].C_LCL,
                                middleValue: _result.groupdata[i].Cbar,
                                maxValue: _result.groupdata[i].C_UCL,
                                Isigma:null,
        ...
        GetDataSeries:function(){
            var Me=this;
            var DataSeries=Agi.Controls.CustomCChartGetDataSries(Me.Get('ChartSeries'),true);
            return DataSeries;
        ...
        ShowALLPlotLineGroup:function(){
            ...
            if(_PlotLineGrps!=null && _PlotLineGrps.length>0){
                for(var i=0;i<_PlotLineGrps.length;i++){
                    newGroup=Agi.Controls.CustomCChartFormatPlotGrpPars(Me.Get("Entity")[0],_PlotLineGrps[i]);//新增标准线组
                    if(newGroup!=null){
            ...
            var DataSeries=Agi.Controls.CustomCChartGetDataSries(chartSeries);
            Agi.Controls.CustomCChartPlotLineUpSeries(DataSeries,PlotLines);//更新Series 点数据
            ...
            if(WarnRuleValue!=null && WarnRuleValue.warnColumn!=null && FilterData!=null && FilterData.length>0){
                ...
                if(checkDataSeries!=null && checkDataSeries.length>0){
                    ...
                    for(var j=0;j<DataSeries[0].data.length;j++){
                        PointRows= Agi.Controls.CustomCChart.FindDataRowsByPoint(checkDataSeries,DataSeries[0].data[j].x);
...
//            debugger;
        ...
        ApplySigmRule:function(_SeriesDataArray){
            ...
            if(SigmArray!=null && SigmArray.length>0){
                ...
                var ojb = new CCRclsSigmaFunc(_SeriesDataArray,SigmArray,CheckDataSries);
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            this.AttributeList = [];
            this.Set("Entity", []);
            this.Set("ControlType", "CustomCChart");
        ...
        SetChartProperty: function (_objCanvasAreaID) {//初始化定义图形控件属性结构，并预设值属性，_objCanvasAreaID：画布显示区域ID
            ...
            var chartProperty = {
                ...
                yAxis_Title_Text: 'C 图', //'单独值', //Y轴标题文字
                ...
                series_MinPlotLine_Names: "C_LCL", //是否显示最低参考线 markeluo 20121126 LCL与UCL 写反了
                series_MiddlePlotLine_Color: "green", //中间值参考线颜色
                series_MiddlePlotLine_Enabled: true, //是否显示中间参考线
                series_MiddlePlotLine_Names: "Cbar", //是否显示最低参考线
                series_MaxPlotLine_Color: "#f41d12", //最大参考线颜色
                series_MaxPlotLine_Enabled: true, //是否显示最大参考线
                series_MaxPlotLine_Names: "C_UCL", //是否显示最低参考线 markeluo 20121126 LCL与UCL 写反了
        ...
        SetChartDataProperty: function () {//数据基本结构定义，在警戒范围中生成模拟数据
            ...
            {//数据结构生成
                ...
                for (var i = 0; i < checkDataSeries.length; i++) {
                    ...
                    Agi.Controls.CustomCChart.MinMaxLineData(obj,dataMin,true,"min");
                    if (i != checkDataSeries.length - 1) dataMin.push(null);

                    Agi.Controls.CustomCChart.MinMaxLineData(obj,dataMiddle,false,"midd")
                    if (i != checkDataSeries.length - 1) dataMiddle.push(null);

                    //判断并填充上下标准线的Series 点
                    Agi.Controls.CustomCChart.MinMaxLineData(obj,dataMax,false,"max");
        ...
        PointClick:function(_PointValue){
            ...
            var GridData={
                ...
                AlarmCells:[], //报警点信息
                AbnormalRows:Agi.Controls.CustomCChart.FindDataRowsByPoint(checkDataSeries,PointRow)//所选的异常点
            };
...
//                {GroupName:"标准线2",MaxSize:1,MaxColor:'#04fe45',MaxValueType:0,MaxValue:29000,MinSize:1,MinColor:'#fbf806',MinValueType:0,MinValue:25800, MaxReviseSig:上限修正符号,MaxReviseValue:上限修正值,MinReviseSig:下限修正符号,MinReviseValue:下限修正值}]);//添加标准线
        ...
        CustomProPanelShow: function () {//显示自定义属性面板
            Agi.Controls.CustomCChartProrityInit(this);
        },
        Destory: function () {
...
//            Agi.Controls.ControlDestoryByList(this);//移除控件,从列表中
        ...
        ControlAttributeChangeEvent: function (_obj, Key, _Value) {
            Agi.Controls.CustomCChartAttributeChange(this, Key, _Value);
        },
        GetConfig: function () {
            var ProPerty = this.Get("ProPerty");
...
//                    alert(JSON.stringify(rowData));
        ...
        SPCViewStandLineMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomCChartView_StandLinesMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，标准线
        SPCViewSigmaMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomCChartView_SigmaMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，西格玛设置
        SPCViewDataExtractMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomCChartView_ExtractMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，数据钻取
        SPCViewChartSetting:function(_Panel,_CallFun){
            ...
            $(_Panel).find(".content").height(200);
            Agi.Controls.CustomCChartView_ChartSetMenu(_Panel, Me);
            _CallFun();
        },//图形选项
...
/*下拉列表控件参数更改处理方法*/
Agi.Controls.CustomCChartAttributeChange = function (_ControlObj, Key, _Value) {
...
//拖拽控件至编辑页面时，初始控件的方法
Agi.Controls.InitCustomCChart = function () {
    return new Agi.Controls.CustomCChart();
}

Agi.Controls.CustomCChartProrityInit = function (_BasicChart) {
    var Me = _BasicChart;
    //取得默认属性
    var cp = Me.Get('ChartProperty');
    ...
    //1.将属性项加入集合,有多少个属性就需要new对应个数的 Agi.Controls.Property.PropertyItem
    {
        ...
        Agi.Controls.CustomCChartView_ChartSetMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomCChartView_BackgroundSetMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomCChartView_StandLinesMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomCChartView_SigmaMenu(ItemContent,Me);
...
//        Agi.Controls.CustomCChartView_ExtractMenu(ItemContent,Me);
...
//1.格式化标准线组的显示值信息
Agi.Controls.CustomCChartFormatPlotGrpPars=function(_Entity,_PlotGrp){
    var PlotArray=null;
...
Agi.Controls.CustomCChartPlotLineUpSeries=function(_SeriesArrar,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
//    var DataSeriesArray=Agi.Controls.CustomCChartGetDataSries(_SeriesArrar);
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
            Agi.Controls.CustomCChartPlotLineUpPoint(_SeriesArrar[i],_PlotLines);
        }
    }
}
Agi.Controls.CustomCChartGetDataSries=function(_SeriesArrar,_IsDataSeries){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    var SeriesArray=[];
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
...
Agi.Controls.CustomCChartPlotLineUpPoint=function(_Series,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    if(_Series!=null && _Series.data!=null && _Series.data.length>0){
        var PointColor=null;
        for(var i=0;i<_Series.data.length;i++){
            PointColor=Agi.Controls.CustomCChartPlotLineGetPointColor(_Series.data[i].y,_PlotLines);
            if(PointColor!=null){
...
Agi.Controls.CustomCChartPlotLineGetPointColor=function(_Value,_PlotLines){
    var PointColor=null;
...
//统计信息
Agi.Controls.CustomCChartAlarmStatistical=function(_ALLSeries,_PlotLines){
    ...
    var _Series=Agi.Controls.CustomCChartGetDataSries(_ALLSeries);//获得对应的的数据Series
    var _ScmSeries=Agi.Controls.CustomCChartGetDataSries(_ALLSeries,false);//获得对应的的西格玛线Series
    ...
    if(_Series!=null || _ScmSeries!=null){
        ...
        for(var i=0;i<_Series.length;i++){
            for(var j=0;j<_Series[i].data.length;j++){
                Agi.Controls.CustomCChartAlarmStatisticalByData(j,_Series[i].data[j].y,AlarmStatisticaldata,SumPointNum);//统计标准线所在比例信息
            }
        }
    }
...
Agi.Controls.CustomCChartAlarmStatisticalByData=function(_index,_PointValue,_AlarmStatisticaldata,SumPointNum){
    //标准线超出点计算
...
//统计信息显示
Agi.Controls.CustomCChartAlarmStatisticalPanelShow=function(_AlarmStatisticaldata,_ApendToPanel){
    if($("#CustomCChartAlarmPanel").length>0){
        $("#CustomCChartAlarmPanel").html("");
    }else{
        var CustomCChartAlarmPanelHTML=$("<div id='CustomCChartAlarmPanel' class='AlarmSty'></div>").appendTo($(_ApendToPanel));
        $("#CustomCChartAlarmPanel").css({"right":"8px","top":"8px"});
    }
    var SubItemHTML="";
    ...
    $("#CustomCChartAlarmPanel").append(SubItemHTML);
}
...
//2.图形选项
Agi.Controls.CustomCChartView_ChartSetMenu=function(_Panel,_Control){
    var Me=_Control;
    //3.1.初始化数据准备
    var ChartEntity=Me.Get("Entity")[0];
    ...
    ItemContentPanel.load('JS/Controls/CustomCChart/tabTemplates.html #tab-1', function () {
        $(this).find("#CustCCdataColor").spectrum({
            showInput: true,
            showPalette: true,
            palette: [
                ['black', 'white'],
        ...
        $(this).find("#CustCCmarkerColor").spectrum({
            showInput: true,
            showPalette: true,
            palette: [
                ['black', 'white'],
        ...
        //默认选中
        $(this).find("#CustCCdataColor").spectrum("set","#d2d2d2");
        $(this).find("#CustCCmarkerColor").spectrum("set","#4572A7");
        if(cp.series_DataColor!=null && cp.series_PointMarkerColor!=null){
            $(this).find("#CustCCdataColor").spectrum("set",cp.series_DataColor);
            $(this).find('#CustCCmarkerColor').spectrum("set",cp.series_PointMarkerColor);
        }
        //分组列和数据列绑定
        var GroupColumns=$(this).find('#CustCCGrpColumn');
        var DataColumns=$(this).find('#CustCCDataColumn');
        ...
        if(cp.SeriesGroupColumn && cp.SeriesDataColumn!=null){
            ...
            if(cp.SeriesGroupColumn==null){
                cp.SeriesGroupColumn=$($("#CustCCGrpColumn").children()[0]).val();
            }
            if(cp.SeriesDataColumn==null){
                cp.SeriesDataColumn=$($("#CustCCDataColumn").children()[0]).val();
            }
        }
        ...
        var CustCChartSeriesItems= $(this).find("#CustCCSeries").empty();
        if(ChartDataSeries!=null && ChartDataSeries.length>0){
            for(var i=0;i<ChartDataSeries.length;i++) {
                ...
                CustCChartSeriesItems.append(option);
            }
            CustCChartSeriesItems.find("option[value='"+ChartDataSeries[0].name+"']").attr("selected","selected");
        }

        //保存处理
        $(this).find("#CustCCSeriesSave").unbind().bind("click",function(ev){
            var CustCChartSeriesObj={
                SeriesName:$("#CustCCSeries").val(),
                GroupColumn:$("#CustCCGrpColumn").val(),
                DataColumn:$("#CustCCDataColumn").val(),
                SeriesColor:$("#CustCCdataColor").val(),
                SeriesMarkerColor:$("#CustCCmarkerColor").val(),
                GroupSize:{
                    Type:0,//类型
            ...
            if($("#NP_SubGrp_0").is(":checked")){}else{
                CustCChartSeriesObj.GroupSize.Type=1;
                CustCChartSeriesObj.GroupSize.Value=$("#NP_SubGrpColumns").val();
            }
            Me.UpSeriesBindInfo(CustCChartSeriesObj);//更新Series 设置
...
Agi.Controls.CustomCChartView_StandLinesMenu=function(_Panel,_Control){
    var Me=_Control;
    //3.1.初始化数据准备
    var ChartEntity=Me.Get("Entity")[0];
    var cp=Me.Get("ChartProperty");//获取属性
    ...
    //3.2.加载标准线设置菜单
    ItemContentPanel.load('JS/Controls/CustomCChart/tabTemplates.html #tab-2', function () {
        //0.控件当前值获取
        ...
        $(this).find("#btnSaveGrp").click(function(){
            ...
            var line={
                GroupName:$("#CustCCStLinename").attr('value'),
            ...
            cp.ChartisigmaValue=$("#CChart_BZSize").val();
        ...
        if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
            $("#CChart_BZSize").val(cp.ChartisigmaValue);
...
Agi.Controls.CustomCChartView_SigmaMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomCChart/tabTemplates.html #tab-3', function () {
        var THisChartSigmaRules=Me.Get("Sigmrules");//获取八项判异规则
        //1.八项规则保存
        $(this).find("#CustCCSigMaRuleSave").unbind().bind("click",function(ev){
            var rule=[];
            var ruleitem=null;
            for(i=1;i<=4;i++)
            {
...
//endreigon
//6.数据钻取设置
Agi.Controls.CustomCChartView_ExtractMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomCChart/tabTemplates.html #tab-4', function () {
        //0.默认选项隐藏
        $("#CustomCCParCancel").hide();
        $("#CustomCCNewParsTxt").width(0).hide();

        //1.添加参数
        $(this).find('#CustomCCParAdd').click(function(){
            $(this).hide();
            $("#CustomCCParCancel").show();
            $("#CustomCCNewParsTxt").width(60).show();
            $("#CustomCCParsNames").hide();
        });
        //2.取消添加
        $(this).find('#CustomCCParCancel').click(function(){
            $("#CustomCCParCancel").hide();
            $("#CustomCCNewParsTxt").width(0).val("").hide();
            $("#CustomCCParsNames").show();
            $("#CustomCCParAdd").show();
        });
        //3.保存
        $(this).find('#CustomCCParSave').click(function(){
            var SCSTxtVisibly=!$("#CustomCCNewParsTxt").is(':hidden');
            var CusChartParsName="";
            var CusChartParsValue="";
            if(SCSTxtVisibly){
                //新增参数
                CusChartParsName=$("#CustomCCNewParsTxt").val();
                CusChartParsValue=$("#CustomCCParsValues").val();
            }else{
                //编辑参数
                CusChartParsName=$("#CustomCCParsNames").val();
                CusChartParsValue=$("#CustomCCParsValues").val();
            }
                ...
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    ...
                    if(SCSTxtVisibly){
                        if(ParIndex>-1){
                            ...
                            $("#CustomCCParsNames").append("<option value='"+CusChartParsName+"'>"+CusChartParsName+"</option>");
                            ...
                            $("#CustomCCParsNames").val(CusChartParsName);
                            //更新列表显示
                            Agi.Controls.CustomCChartParsListUpdate(ChartExtractConfig.Paras,"ParsListPanel");
        ...
        //4.删除
        $(this).find('#CustomCCParDel').click(function(){
            var SCSTxtVisibly=!$("#CustomCCNewParsTxt").is(':hidden');
            if(SCSTxtVisibly){
                AgiCommonDialogBox.Alert("请取消或完成新增，选择已存在的参数名称后再试!");
            }else{
                //编辑参数
               var CusChartParsName=$("#CustomCCParsNames").val();
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    var ParIndex=-1;
                    for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                        if(ChartExtractConfig.Paras[i].Name==CusChartParsName){
                    ...
                    if(ParIndex>-1){
                        //移除对应的Option项
                        $("#CustomCCParsNames option[value='"+ChartExtractConfig.Paras[ParIndex].Name+"']").remove(); //删除Select中Value=''的Option
        ...
        Agi.PageDataDrill.GetAllPageFilList(function(_result){
            if(_result.result && _result.result=="true"){
                ...
                if(_result.data!=null && _result.data.length>0){
                    ...
                    itemValue=null;
                    $("#CustCCExtractURL").html(strCustSigChartExtractURLoptions);
                    if(ChartExtractConfig!=null && ChartExtractConfig.URL!=null){
                        $("#CustCCExtractURL").find("[value='"+ChartExtractConfig.URL+"']").attr("selected","selected");
                    }
                }
            }
        })
        if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
            for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                $("#CustomCCParsNames").append("<option value='"+ChartExtractConfig.Paras[i].Name+"'>"+ChartExtractConfig.Paras[i].Name+"</option>");
            }
            DefaultParNameColumn=ChartExtractConfig.Paras[0].Column;
            $("#CustomCCParsNames").val(ChartExtractConfig.Paras[0].Name);
        ...
        if(ChartEntityColumns!=null && ChartEntityColumns.length>0){
            for(var i=0;i<ChartEntityColumns.length;i++){
                $("#CustomCCParsValues").append("<option value='"+ChartEntityColumns[i]+"'>"+ChartEntityColumns[i]+"</option>");
            }
            if(DefaultParNameColumn!=null){
                $("#CustomCCParsValues").val(DefaultParNameColumn);
            }
        }
        //6.参数名称选中项更改
        $(this).find('#CustomCCParsNames').change(function(){
            ...
            if(ThisParas!=null && ThisParas.length>0){
                for(var i=0;i<ThisParas.length;i++){
                    if(ThisParas[i].Name==StrThisSelValue){
                        $("#CustomCCParsValues").val(ThisParas[i].Column);
                        break;
                    }
                }
            }
        ...
        $(this).find('#CstNPExtractOptionSave').click(function(){
            ...
            ThisExtractConfig.URL=$("#CustCCExtractURL").val();
...
//7.背景设置
Agi.Controls.CustomCChartView_BackgroundSetMenu=function(_Panel,_Control){
    var Me=_Control;
    //获取属性
    var cp = Me.Get('ChartProperty');
    ...
    ItemContentPanel.load('JS/Controls/CustomCChart/tabTemplates.html #tab-5', function () {
        ...
        $("#CustomCC_bgvalue").unbind().bind("click",function(){
        ...
        var ThisChartbgvalue=Agi.Controls.CustomCChartBackgroundValueGet(cp.ChartBackConfig.backgroundColor);
        $("#CustomCC_bgvalue").css("background",ThisChartbgvalue.value.background).data('colorValue',ThisChartbgvalue);//设置默认项
        // backgroundColor//背景 ,Tolbtnbackground,//菜单按钮背景,TolbtnFontColor,//菜单按钮字体,TolTabbackground,//统计表格背景,TolTabFontColor//统计表格字体
        $("#CustomCC_Tolbgvalue").spectrum("set",cp.ChartBackConfig.Tolbtnbackground);
        $("#CustomCC_Tolbtncolor").spectrum("set",cp.ChartBackConfig.TolbtnFontColor);
        $("#CustomCC_TolTabbg").spectrum("set",cp.ChartBackConfig.TolTabbackground);
        $("#CustomCC_TolTabfontbg").spectrum("set",cp.ChartBackConfig.TolTabFontColor);

        $("#CustomCC_BackgroundSave").unbind().bind("click",function(){
            var color=$("#CustomCC_bgvalue").data('colorValue');
            ...
            Agi.Controls.CustomCChart.BackgroundApply(Me,BackgroudObj);//背景应用
            Me.RefreshByProPanelUp();//更新属性后，刷新图形显示
        })
    });
}
...
//region 编辑数据
//显示单值图数据钻取参数列表
Agi.Controls.CustomCChartParsListUpdate=function(_Paras,_ListPanelID){
    var StrListItems="";
    if(_Paras!=null && _Paras.length>0){
        for(var i=0;i<_Paras.length;i++){
            StrListItems+=" <div class='ParsListHeadSty'><div class='CustomCCParsListCellSty'>"+_Paras[i].Name+"</div><div class='CustomCCParsListCellSty'>"+_Paras[i].Column+"</div></div>";
        }
    }
...
//endregion
//region 背景设置
Agi.Controls.CustomCChartBackgroundValueGet=function(_oBackgroundObj){
...
Agi.Controls.CustomCChart.BackgroundApply=function(_ControlObj,_BackGroudObj){
...
Agi.Controls.CustomCChart.FindDataRowsByPoint=function(_checkDataSeries,_PointX){
//    var checkDataSeries = this.Get('CheckDataSeries');
    var PointRows=[];
...
Agi.Controls.CustomCChart.MinMaxLineData=function(_ThischeckSeriesObj,_MinMaxData,_IsMin,_linetype){
    if(_ThischeckSeriesObj.minValue.length>1){
        ...
        for(var i=0;i<_ThischeckSeriesObj.Data.length;i++){
            ...
            switch(_linetype){
                ...
                case "max":
                    ...
                    break;
                case "midd":
                    _MinMaxData.push([stepIndex1, _ThischeckSeriesObj.middleValue]);
                    _MinMaxData.push([stepIndex2, _ThischeckSeriesObj.middleValue]);
                    break;
                default:
                    break;
            }
        ...
        switch(_linetype){
            ...
            case "max":
                ...
                break;
            case "midd":
                _MinMaxData.push([_ThischeckSeriesObj.min, _ThischeckSeriesObj.middleValue]);
                _MinMaxData.push([_ThischeckSeriesObj.max, _ThischeckSeriesObj.middleValue]);
                break;
            default:
                break;
        }
...
//曲线Series 计算
Agi.Controls.CustomCChart.SeriesDataCal=function(_DataArray,_GroupValue){
    var TempDataArray=[];
