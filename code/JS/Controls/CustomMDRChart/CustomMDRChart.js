...
Agi.Controls.CustomMDRChart = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
...
//            cp.series_MiddlePlotLine_Enabled = false;
        ...
        ChartLoadByData:function(_Data){
            ...
            if(_Data!=null && _Data.length>0){
                ...
                Me.Set("GroupLaves",Agi.Controls.CustomMDRChart.GroupSizeDataSave(DataArray,cp.chart_Data_NRow));

                var jsonData = { "action": "RCalMedianRange",
                    "name": ArrayBCName,
                    "dataArray": DataArray,
                ...
                Agi.DAL.ReadData({ "MethodName": "RCalMedianRange", "Paras": jsonString, "CallBackFunction": function (_result) {
                    if (_result && _result.result == "true") {
                        ...
                        if (_result.Median_data != null &&
                            _result.name != null &&
                            _result.Median_data.length == _result.name.length) {
                            for (var i = 0; i < _result.Median_data.length; i++) {
                                var min = checkDataSeries.Mean_data.length > 0 ? checkDataSeries.Mean_data[checkDataSeries.Mean_data.length - 1].max : 0;
                                var max = min + _result.Median_data[i].values.length;
                                checkDataSeries.Mean_data.push({
                                    Names: _result.name[i],
                                    min: min,
                                    max: max,
                                    minValue: _result.Median_data[i].LCL,
                                    middleValue: _result.Median_data[i].XBarBar,
                                    maxValue: _result.Median_data[i].UCL,
                                    Isigma: _result.Median_data[i].Msigma,
                                    LaveNum:GroupLaves.Items[i],
                                    Nrow:GroupLaves.Nrow,
                                    Data: _result.Median_data[i].values
                                })
                            }
                        }
                        else {
                        ...
                        if (_result.R_data != null &&
                            _result.name != null &&
                            _result.R_data.length == _result.name.length) {
                            for (var i = 0; i < _result.R_data.length; i++) {
                                var min = checkDataSeries.SD_data.length > 0 ? checkDataSeries.SD_data[checkDataSeries.SD_data.length - 1].max : 0;
                                var max = min + _result.R_data[i].values.length;
                                checkDataSeries.SD_data.push({
                                    Names: _result.name[i],
                                    min: min,
                                    max: max,
                                    minValue: _result.R_data[i].LCL,
                                    middleValue: _result.R_data[i].RBar,
                                    maxValue: _result.R_data[i].UCL,
                                    Isigma: _result.R_data[i].Rsigma,
                                    LaveNum:GroupLaves.Items[i],
                                    Nrow:GroupLaves.Nrow,
                                    Data: _result.R_data[i].values
                                })
                            }
                        }
                        else {
        ...
        UpPlotLineGroups:function(_PlotLineGrps){
            //[{GroupName:"标准线1",MaMDRize:1,MaxColor:'red',MaxValueType:0,MaxValue:3000,MinSize:1,MinColor:'red',MinValueType:0,MinValue:3000, MaxReviseSig:上限修正符号,MaxReviseValue:上限修正值,MinReviseSig:下限修正符号,MinReviseValue:下限修正值}]
        ...
        UpSigmConditions:function(_rules){
            var Me=this;
            Me.Set("SigMDRules",_rules);//保存八项判异规则
        ...
        GetDataSeries:function(){
            ...
            var DataSeries={
                    Mean_data:Agi.Controls.CustomMDRChartGetDataSries(TwoChartSeries.Mean_data,true),
                    SD_data:Agi.Controls.CustomMDRChartGetDataSries(TwoChartSeries.SD_data,true)
                }
            return DataSeries;
        ...
        ShowALLPlotLineGroup:function(){
            ...
            //region 1.均值图
            var CusMDRChart_Mean=Me.Get('chart');
            var PlotLineAndBands= CusMDRChart_Mean.yAxis[0].plotLinesAndBands[0];
            if(PlotLineAndBands!=null&& PlotLineAndBands.length>0){
                for(var i=0;i<PlotLineAndBands.length;i++){
                    CusMDRChart_Mean.yAxis[0].removePlotLine(PlotLineAndBands[i].id);
                }
            }
            ...
            if(_PlotLineGrps!=null && _PlotLineGrps.length>0){
                for(var i=0;i<_PlotLineGrps.length;i++){
                    newGroup=Agi.Controls.CustomMDRChartFormatPlotGrpPars(Me.Get("Entity")[0],_PlotLineGrps[i]);//新增标准线组
                    if(newGroup!=null){
            ...
            if(PlotLines!=null){
                //获取当前标准线组的标准线元素
                var Yasixobj=CusMDRChart_Mean.yAxis[0];
                for(var i=0;i<PlotLines.length;i++){
            ...
            var DataSeries=Agi.Controls.CustomMDRChartGetDataSries(chartSeries.Mean_data);
            Agi.Controls.CustomMDRChartPlotLineUpSeries(DataSeries,PlotLines);//更新Series 点数据
            ...
            if(WarnRuleValue!=null && WarnRuleValue.warnColumn!=null && FilterData!=null && FilterData.length>0){
                ...
                if(checkDataSeries.Mean_data!=null && checkDataSeries.Mean_data.length>0){
                    ...
                    for(var j=0;j<DataSeries[0].data.length;j++){
                        PointRows= Agi.Controls.CustomMDRChart.FindDataRowsByPoint(checkDataSeries.Mean_data,DataSeries[0].data[j].x);
            ...
            if(DataSeries!=null && DataSeries.length>0){
                for(var i=0;i<CusMDRChart_Mean.series.length;i++){
                    for(var j=0;j<DataSeries.length;j++){
                        if(CusMDRChart_Mean.series[i].name==DataSeries[j].name){
                            CusMDRChart_Mean.series[i].remove();
                            i=-1;
                            break;
                        }
                    }
                ...
                for(var i=0;i<DataSeries.length;i++){
                    DataSeries[i].data=Me.ApplySigMDRule(DataSeries[i].data,checkDataSeries.Mean_data);//应用西格玛八项判异规则
                    CusMDRChart_Mean.addSeries(DataSeries[i]);
                }
            }
           //endregion

            //region 2.极差图
            var CusMDRChart_SD=Me.Get('chart_SD');
            var PlotLineAndBands= CusMDRChart_SD.yAxis[0].plotLinesAndBands[0];
            if(PlotLineAndBands!=null&& PlotLineAndBands.length>0){
                for(var i=0;i<PlotLineAndBands.length;i++){
                    CusMDRChart_SD.yAxis[0].removePlotLine(PlotLineAndBands[i].id);
                }
            }
            ...
            if(PlotLines_sd!=null){
                //获取当前标准线组的标准线元素
                var Yasixobj=CusMDRChart_SD.yAxis[0];
            ...
            var DataSeries=Agi.Controls.CustomMDRChartGetDataSries(chartSeries.SD_data);
            Agi.Controls.CustomMDRChartPlotLineUpSeries(DataSeries,PlotLines_sd);//更新Series 点数据
            ...
            if(WarnRuleValue!=null && WarnRuleValue.warnColumn!=null && FilterData!=null && FilterData.length>0){
                ...
                if(checkDataSeries.SD_data!=null && checkDataSeries.SD_data.length>0){
                    var PointRows=null;
                    for(var j=0;j<DataSeries[0].data.length;j++){
                        PointRows= Agi.Controls.CustomMDRChart.FindDataRowsByPoint(checkDataSeries.SD_data,DataSeries[0].data[j].x);
                        bool=Agi.Controls.WarnValueCompare(FilterData[PointRows[0]][WarnRuleValue.warnColumn],WarnRuleValue.warnCompareValue,WarnRuleValue.warnRule);
                        if(bool){
                            DataSeries[0].data[j].marker={fillColor:WarnRuleValue.warnColor};
                        }
            ...
            if(DataSeries!=null && DataSeries.length>0){
                for(var i=0;i<CusMDRChart_SD.series.length;i++){
                    for(var j=0;j<DataSeries.length;j++){
                        if(CusMDRChart_SD.series[i].name==DataSeries[j].name){
                            CusMDRChart_SD.series[i].remove();
                            i=-1;
                            break;
                        }
                    }
                ...
                for(var i=0;i<DataSeries.length;i++){
                    DataSeries[i].data=Me.ApplySigMDRule(DataSeries[i].data,checkDataSeries.SD_data);//应用西格玛八项判异规则
                    CusMDRChart_SD.addSeries(DataSeries[i]);
                }
            }
            //endregion

//           //标准线、西格玛线 统计信息
//           var tolInfo=Agi.Controls.CustomMDRChartAlarmStatistical(chartSeries,PlotLines);//统计信息
//           var AppendToPanel=Me.Get("HTMLElement");
//            var cp=Me.Get('ChartProperty');
//            tolInfo.SGM.SigmaValue=cp.ChartisigmaValue;//西格玛系数
//            Agi.Controls.CustomMDRChartAlarmStatisticalTableShow(tolInfo,AppendToPanel,cp.ChartBackConfig);
        },//显示标准线
...
//            debugger;
        ...
        ApplySigMDRule:function(_SeriesDataArray,_CheckDataSeries){
            var Me=this;
            var SigmArray= Me.Get("SigMDRules");//八项判异规则数组
            var NewSeriesData=_SeriesDataArray;
            if(SigmArray!=null && SigmArray.length>0){
                ////待实现
                //NewSeriesData=SigmArlmFunc({SeriesData:_SeriesDataArray,SigMDRule:SigmArray,GroupData:CheckDataSries});
                var NewCheckDataSries=Agi.Controls.SigMDRuleApplyGetGroupData(_CheckDataSeries);//分组数据信息
                var ojb = new MDRclsSigmaFunc(_SeriesDataArray,SigmArray,NewCheckDataSries);
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            this.AttributeList = [];
            this.Set("Entity", []);
            this.Set("ControlType", "CustomMDRChart");
            ...
            var ID = savedId ? savedId : "CustomMDRChart" + Agi.Script.CreateControlGUID();
            ...
            this.Set("SigMDRules",DefaultruleArray);
        ...
        SetChartProperty: function (_objCanvasAreaID,_objCanvasSDID) {//初始化定义图形控件属性结构，并预设值属性，_objCanvasAreaID：画布显示区域ID
            ...
            var chartProperty = {
                ...
                yAxis_Title_Text: '中位数', //Y轴标题文字,均值图
                yAxis_Title_Text_SD: '样本极差', //Y轴标题文字,极差图
                ...
                series_MiddlePlotLine_Names_SD: "R", //是否显示最低参考线,极差图
            ...
            if(checkDataSeries.Mean_data!=null && checkDataSeries.Mean_data.length>0){
                {//数据结构生成
                    ...
                    for (var i = 0; i < checkDataSeries.Mean_data.length; i++) {
                        ...
                        obj.Data = Agi.Controls.CustomMDRChart.GetData(obj,cp); //获取模拟数据
        ...
        PointClick:function(_PointValue,_ChartNO){
            ...
            var GridData={
                ...
                AlarmCells:[], //报警点信息
                AbnormalRows:Agi.Controls.CustomMDRChart.FindDataRowsByPoint(ThisChartCheckSeries,PointRow)//所选的异常点
            };
            ...
            if(ThisSelChartSeries!=null && ThisSelChartSeries.length>0){
                for(var i=0;i<ThisSelChartSeries.length;i++){
                    Agi.Controls.CustomMDRChartAlarmDataPointsFind(GridData,cp.SeriesDataColumn[i],ThisSelChartSeries[i],cp.series_PointMarkerColor,ThisChartCheckSeries);
                }
            }
        ...
        CustomProPanelShow: function () {//显示自定义属性面板
            Agi.Controls.CustomMDRChartProrityInit(this);
        },
        Destory: function () {
...
//            Agi.Controls.ControlDestoryByList(this);//移除控件,从列表中
        ...
        ControlAttributeChangeEvent: function (_obj, Key, _Value) {
            Agi.Controls.CustomMDRChartAttributeChange(this, Key, _Value);
        },
        GetConfig: function () {
            var ProPerty = this.Get("ProPerty");
...
//            });
            ...
            //均值极差图新增属性
            SPCSingleChartControl.Control.SigmaRule=this.Get("SigMDRules");//西格玛规则
        ...
        CreateControl: function (_Config, _Target) {
            ...
            if (_Config != null) {
                ...
                if (_Target != null && _Target != "") {
                    ...
                    this.Set("SigMDRules",_Config.SigmaRule);//西格玛规则
...
//                    alert(JSON.stringify(rowData));
        ...
        SPCViewStandLineMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomMDRChartView_StandLinesMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，标准线
        SPCViewSigmaMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomMDRChartView_SigmaMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，西格玛设置
        SPCViewDataExtractMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomMDRChartView_ExtractMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，数据钻取
        SPCViewSpecialAlarmMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomMDRChartView_SpecialAlarmMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，特殊报警
        GetSPCViewGridData:function(){
            ...
            if(ThisDataSeries!=null && ThisDataSeries.Mean_data.length>0){
                for(var i=0;i<ThisDataSeries.Mean_data.length;i++){
                    Agi.Controls.CustomMDRChartAlarmDataPointsFind(GridData,cp.SeriesDataColumn[i],ThisDataSeries.Mean_data[i],cp.series_PointMarkerColor,checkDataSeries.Mean_data);
                }
            }
            if(ThisDataSeries!=null && ThisDataSeries.SD_data.length>0){
                for(var i=0;i<ThisDataSeries.SD_data.length;i++){
                    Agi.Controls.CustomMDRChartAlarmDataPointsFind(GridData,cp.SeriesDataColumn[i],ThisDataSeries.SD_data[i],cp.series_PointMarkerColor,checkDataSeries.SD_data);
                }
            }
            return GridData;
        ...
        SPCViewRefreshByGridData:function(_GridData){
            ...
            //setTimeout(Agi.Controls.CustomMDRChartLastChartShowExE(Me),2000);
            Agi.Controls.CustomMDRChartLastChartShow(Me);
            if(Me.Get("LastChartImgIsExt")){
                Me.RefreshByProPanelUp();//更新显示
            }else{
                setTimeout(Agi.Controls.CustomMDRChartRefreshShowExe(Me),2000);//更新显示
            }
            //Me.RefreshByProPanelUp();//更新显示
        ...
        SPCViewDataRestore:function(){
            ...
            Agi.Controls.CustomMDRChartLastChartShowMenu(Me,false);//还原数据后，清除原对照图
...
/*下拉列表控件参数更改处理方法*/
Agi.Controls.CustomMDRChartAttributeChange = function (_ControlObj, Key, _Value) {
...
//拖拽控件至编辑页面时，初始控件的方法
Agi.Controls.InitCustomMDRChart = function () {
    return new Agi.Controls.CustomMDRChart();
}

Agi.Controls.CustomMDRChartProrityInit = function (_BasicChart) {
    var Me = _BasicChart;
    //取得默认属性
    var cp = Me.Get('ChartProperty');
    ...
    //1.将属性项加入集合,有多少个属性就需要new对应个数的 Agi.Controls.Property.PropertyItem
    {
        ...
        ItemContent.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-1', function () {
            $(this).find("#CustMDRdataColor").spectrum({
                showInput: true,
                showPalette: true,
                palette: [
                    ['black', 'white'],
            ...
            $(this).find("#CustMDRmarkerColor").spectrum({
                showInput: true,
                showPalette: true,
                palette: [
                    ['black', 'white'],
            ...
            //默认选中
            $(this).find("#CustMDRdataColor").spectrum("set","#d2d2d2");
            $(this).find("#CustMDRmarkerColor").spectrum("set","#4572A7");
            if(cp.series_DataColor!=null && cp.series_PointMarkerColor!=null){
                $(this).find("#CustMDRdataColor").spectrum("set",cp.series_DataColor);
                $(this).find('#CustMDRmarkerColor').spectrum("set",cp.series_PointMarkerColor);
            }
            //分组列和数据列绑定
            var GroupColumns=$(this).find('#CustMDRGrpColumn');
            var DataColumns=$(this).find('#CustMDRDataColumn');
            GroupColumns.empty();
            DataColumns.empty();
            ...
            if(cp.SeriesGroupColumn && cp.SeriesDataColumn!=null){
                ...
                if(cp.SeriesGroupColumn==null){
                    cp.SeriesGroupColumn=$($("#CustMDRGrpColumn").children()[0]).val();
                }
                if(cp.SeriesDataColumn==null){
                    cp.SeriesDataColumn=$($("#CustMDRDataColumn").children()[0]).val();
                }
            }
            ...
            var CustMDRChartSeriesItems= $(this).find("#CustMDRSeries").empty();
            if(ChartDataSeries!=null && ChartDataSeries.length>0){
                for(var i=0;i<ChartDataSeries.length;i++) {
                    ...
                    CustMDRChartSeriesItems.append(option);
                }
                CustMDRChartSeriesItems.find("option[value='"+ChartDataSeries[0].name+"']").attr("selected","selected");
            }
            //子组大小初始化
            if(cp.chart_Data_NRow==null){cp.chart_Data_NRow=2};
            $("#MDRChart_RGroupSize").val(cp.chart_Data_NRow);

            //保存处理
            $(this).find("#CustMDRSeriesSave").unbind().bind("click",function(ev){
                var CustMDRChartSeriesObj={
                    SeriesName:$("#CustMDRSeries").val(),
                    GroupColumn:$("#CustMDRGrpColumn").val(),
                    DataColumn:$("#CustMDRDataColumn").val(),
                    SeriesColor:$("#CustMDRdataColor").val(),
                    SeriesMarkerColor:$("#CustMDRmarkerColor").val(),
                    GroupSize:$("#MDRChart_RGroupSize").val()//分组大小
                }
                Me.UpSeriesBindInfo(CustMDRChartSeriesObj);//更新Series 设置
        ...
        Agi.Controls.CustomMDRChartView_BackgroundSetMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMDRChartView_SigmaSettingMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMDRChartView_RTypeMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMDRChartView_SigmaMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMDRChartView_StandLinesMenu(ItemContent,Me);
        ThisProItems.push(new Agi.Controls.Property.PropertyItem({ Title: "中位数_标准线", DisabledValue: 1, ContentObj: ItemContent }));
        //endregion

        //region 4.6.2.样本极差标准线
        ItemContent = null;
        ItemContent=$("<div id='CSChartStandLine_SD'></div>");
        Agi.Controls.CustomMDRChartView_StandLinesMenu_SD(ItemContent,Me);
        ThisProItems.push(new Agi.Controls.Property.PropertyItem({ Title: "极差_标准线", DisabledValue: 1, ContentObj: ItemContent }));
        //endregion
...
//        ItemContent = null;
//        ItemContent=$("<div id='CSChartSigMa'></div>");
//        Agi.Controls.CustomMDRChartView_SpecialAlarmMenu(ItemContent,Me);
...
//        Agi.Controls.CustomMDRChartView_ExtractMenu(ItemContent,Me);
...
//1.格式化标准线组的显示值信息
Agi.Controls.CustomMDRChartFormatPlotGrpPars=function(_Entity,_PlotGrp){
    var PlotArray=null;
    //_PlotGrp: {GroupName:"标准线1",MaMDRize:1,MaxColor:'red',MaxValueType:0,MaxValue:3000,MinSize:1,MinColor:'red',MinValueType:0,MinValue:3000, MaxReviseSig:上限修正符号,MaxReviseValue:上限修正值,MinReviseSig:下限修正符号,MinReviseValue:下限修正值}
    if(_PlotGrp!=null){
        PlotArray=[];
        ...
        PlotArray.push({value:MaxValue,columnName:maxcolumn,dashStyle:'solid',color:_PlotGrp.MaxColor,width:_PlotGrp.MaMDRize,id:_PlotGrp.GroupName+"_MAX",zIndex: 5});
...
Agi.Controls.CustomMDRChartPlotLineUpSeries=function(_SeriesArrar,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
//    var DataSeriesArray=Agi.Controls.CustomMDRChartGetDataSries(_SeriesArrar);
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
            Agi.Controls.CustomMDRChartPlotLineUpPoint(_SeriesArrar[i],_PlotLines);
        }
    }
}
Agi.Controls.CustomMDRChartGetDataSries=function(_SeriesArrar,_IsDataSeries){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    var SeriesArray=[];
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
...
Agi.Controls.CustomMDRChartPlotLineUpPoint=function(_Series,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    if(_Series!=null && _Series.data!=null && _Series.data.length>0){
        var PointColor=null;
        for(var i=0;i<_Series.data.length;i++){
            PointColor=Agi.Controls.CustomMDRChartPlotLineGetPointColor(_Series.data[i].y,_PlotLines);
            if(PointColor!=null){
...
Agi.Controls.CustomMDRChartPlotLineGetPointColor=function(_Value,_PlotLines){
    var PointColor=null;
...
//统计信息
Agi.Controls.CustomMDRChartAlarmStatistical=function(_ALLSeries,_PlotLines){
    ...
    var _Series=Agi.Controls.CustomMDRChartGetDataSries(_ALLSeries);//获得对应的的数据Series
    var _ScmSeries=Agi.Controls.CustomMDRChartGetDataSries(_ALLSeries,false);//获得对应的的西格玛线Series
    ...
    if(_Series!=null || _ScmSeries!=null){
        ...
        for(var i=0;i<_Series.length;i++){
            for(var j=0;j<_Series[i].data.length;j++){
                Agi.Controls.CustomMDRChartAlarmStatisticalByData(j,_Series[i].data[j].y,AlarmStatisticaldata,SumPointNum);//统计标准线所在比例信息
            }
        }
    }
...
Agi.Controls.CustomMDRChartAlarmStatisticalByData=function(_index,_PointValue,_AlarmStatisticaldata,SumPointNum){
    //标准线超出点计算
...
//统计信息显示
Agi.Controls.CustomMDRChartAlarmStatisticalPanelShow=function(_AlarmStatisticaldata,_ApendToPanel){
    if($("#CustomMDRChartAlarmPanel").length>0){
        $("#CustomMDRChartAlarmPanel").html("");
    }else{
        var CustomMDRChartAlarmPanelHTML=$("<div id='CustomMDRChartAlarmPanel' class='AlarmSty'></div>").appendTo($(_ApendToPanel));
        $("#CustomMDRChartAlarmPanel").css({"right":"8px","top":"8px"});
    }
    var SubItemHTML="";
    ...
    $("#CustomMDRChartAlarmPanel").append(SubItemHTML);
}
...
Agi.Controls.SigMDRuleApplyGetGroupData=function(_CheckedDataArray){
    var GroupData=[];
...
//2.查找异常点
Agi.Controls.CustomMDRChartAlarmDataPointsFind=function(_GridData,_ColumnName,_ChartSeires,_markercolor,_CheckDataSeries){
    var _Index=-1;
    ...
    if(_Index>-1){
        ...
        for(var j=0;j<_ChartSeires.data.length;j++){
            if(_ChartSeires.data[j].marker.fillColor!=_markercolor){//如果marker点颜色与原设置point点颜色不一致则说明该点为报警点
                NewItemArray=Agi.Controls.CustomMDRChart.FindDataRowsByPoint(_CheckDataSeries,_ChartSeires.data[j].x);
...
Agi.Controls.CustomMDRChartView_StandLinesMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-2', function () {
        //0.控件当前值获取
        ...
        $(this).find("#btnNewGrp").click(function(){
            ...
            LineTablePanel.find('#CustMDRStLinename').show();
            LineTablePanel.find('#CustMDRStLinegroupName').hide();
        ...
        $(this).find("#btnSaveGrp").click(function(){
            ...
            if($("#CustMDRStLinegroupName").is(":hidden")){
                if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
                    ...
                    if(Intopstatevalue==0){//新增
                        for(var i=0;i<ThisChartStandLines.length;i++){
                            if($("#CustMDRStLinename").attr('value')==ThisChartStandLines[i].GroupName)
                            {
                        ...
                        var NewGrpName=$("#CustMDRStLinename").attr('value');
                        var OldSelGrpName=$("#CustMDRStLinegroupName").val();
                        var indexA=-1;
                        var indexB=-1;
                ...
                var SelValue=$("#CustMDRStLinegroupName").val();
                if(SelValue!=null && SelValue!=""){
                    SelectedItemObj=SelValue;
                }
            }
            $('#CustMDRStLinename').hide();
            $('#CustMDRStLinegroupName').show();
            $('#btnGrpUndo').hide();
            $('#btnEditGrp').show();
            $("#btnNewGrp").show();
            $("#btnDeleteGrp").show();
            ...
            var line={
                GroupName:$("#CustMDRStLinename").attr('value'),
                MaMDRize:$("#topWidth").attr('value'),
            ...
            var StandLinGrpNames = $("#CustMDRStLinegroupName");
        ...
        $(this).find('#btnDeleteGrp').click(function(){
            if($('#CustMDRStLinegroupName').css('display')!="none"){
                var index=-1;
                for(var i=0;i<ThisChartStandLines.length;i++){
                    if($('#CustMDRStLinegroupName option:selected').text()==ThisChartStandLines[i].GroupName)
                    {
                        index=i;break;
                    }
                }
                ThisChartStandLines.splice(index,1);
                var name = $("#CustMDRStLinegroupName");
                name.empty();
                for(var i=0;i<ThisChartStandLines.length;i++) {
                    var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>"
                    name.append(option);
                ...
                {
                    $('#CustMDRStLinegroupName').hide();
                    $('#CustMDRStLinename').val("").show();
                }
                groupNameChange(ThisChartStandLines);
            }
            else{
        ...
        $(this).find("#btnEditGrp").unbind().bind("click",function(ev){
            var LineTablePanel=$(this).parent().parent().parent();
            LineTablePanel.find('#CustMDRStLinename').show().val(LineTablePanel.find('#CustMDRStLinegroupName').val());
            LineTablePanel.find('#CustMDRStLinegroupName').hide();
        ...
        $(this).find("#CustMDRStLinegroupName").change(function(){
            debugger;
            groupNameChange(ThisChartStandLines);
        });
        ...
        var groupNameChange=function(_StandLines){
            ...
            {
                if($("#CustMDRStLinegroupName  option:selected").text()==_StandLines[i].GroupName){
                    $('#topWidth').attr('value',_StandLines[i].MaMDRize);
        ...
        //4.初始化绑定
        var StandLinGrpNames = $(this).find("#CustMDRStLinegroupName");
        StandLinGrpNames.empty();
        if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
            for(var i=0;i<ThisChartStandLines.length;i++) {
                var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>";
            ...
            StandLinGrpNames.find("option[value='"+ThisChartStandLines[0].GroupName+"']").attr("selected",true);
            $(this).find('#CustMDRStLinename').hide();
            $(this).find('#CustMDRStLinegroupName').show();
            groupNameChange(ThisChartStandLines);//绑定显示

            $("#btnEditGrp").show();
        }else{
            $(this).find('#CustMDRStLinename').show();
            $(this).find('#CustMDRStLinegroupName').hide();
        }
...
//3.2.样本极差标准线
Agi.Controls.CustomMDRChartView_StandLinesMenu_SD=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-10', function () {
        //0.控件当前值获取
        ...
        $(this).find("#btnNewGrp_SD").click(function(){
            //清空面板
            var LineTablePanel=$(this).parent().parent().parent();
            LineTablePanel.find('#CustMDRStLinename_SD').show();
            LineTablePanel.find('#CustMDRStLinegroupName_SD').hide();
        ...
        $(this).find("#btnSaveGrp_SD").click(function(){
            ...
            if($("#CustMDRStLinegroupName_SD").is(":hidden")){
                if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
                    //创建对象
                    if(Intopstatevalue==0){//新增
                        for(var i=0;i<ThisChartStandLines.length;i++){
                            if($("#CustMDRStLinename_SD").attr('value')==ThisChartStandLines[i].GroupName)
                            {
                                AgiCommonDialogBox.Alert('标准线不允许重名！');
                                return;
                            }
                        ...
                        var NewGrpName=$("#CustMDRStLinename_SD").attr('value');
                        var OldSelGrpName=$("#CustMDRStLinegroupName_SD").val();
                        var indexA=-1;
                        var indexB=-1;
                        if(NewGrpName!=OldSelGrpName){
                            for(var i=0;i<ThisChartStandLines.length;i++){
                ...
                var SelValue=$("#CustMDRStLinegroupName_SD").val();
                if(SelValue!=null && SelValue!=""){
                    SelectedItemObj=SelValue;
                }
            }
            $('#CustMDRStLinename_SD').hide();
            $('#CustMDRStLinegroupName_SD').show();
            $('#btnGrpUndo_SD').hide();
            $('#btnEditGrp_SD').show();
            $("#btnNewGrp_SD").show();
            ...
            var line={
                GroupName:$("#CustMDRStLinename_SD").attr('value'),
                MaMDRize:$("#topWidth_SD").attr('value'),
            ...
            var StandLinGrpNames = $("#CustMDRStLinegroupName_SD");
            StandLinGrpNames.empty();
            if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
                for(var i=0;i<ThisChartStandLines.length;i++) {
                    var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>";
        ...
        $(this).find('#btnDeleteGrp_SD').click(function(){
            if($('#CustMDRStLinegroupName_SD').css('display')!="none"){
                var index=-1;
                for(var i=0;i<ThisChartStandLines.length;i++){
                    if($('#CustMDRStLinegroupName_SD option:selected').text()==ThisChartStandLines[i].GroupName)
                    {
                        index=i;break;
                    }
                }
                ThisChartStandLines.splice(index,1);
                var name = $("#CustMDRStLinegroupName_SD");
                name.empty();
                for(var i=0;i<ThisChartStandLines.length;i++) {
                    var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>"
                    name.append(option);
                ...
                if(ThisChartStandLines.length==0)
                {
                    $('#CustMDRStLinegroupName_SD').hide();
                    $('#CustMDRStLinename_SD').val("").show();
                }
                groupNameChange(ThisChartStandLines);
            }
            else{
        ...
        $(this).find("#btnEditGrp_SD").unbind().bind("click",function(ev){
            var LineTablePanel=$(this).parent().parent().parent();
            LineTablePanel.find('#CustMDRStLinename_SD').show().val(LineTablePanel.find('#CustMDRStLinegroupName_SD').val());
            LineTablePanel.find('#CustMDRStLinegroupName_SD').hide();
        ...
        $(this).find("#CustMDRStLinegroupName_SD").change(function(){
            debugger;
            groupNameChange(ThisChartStandLines);
        });
        ...
        var groupNameChange=function(_StandLines){
            for(var i=0;i<_StandLines.length;i++)
            {
                if($("#CustMDRStLinegroupName_SD  option:selected").text()==_StandLines[i].GroupName){
                    $('#topWidth_SD').attr('value',_StandLines[i].MaMDRize);
        ...
        //4.初始化绑定
        var StandLinGrpNames = $(this).find("#CustMDRStLinegroupName_SD");
        StandLinGrpNames.empty();
        if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
            for(var i=0;i<ThisChartStandLines.length;i++) {
                var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>";
            ...
            StandLinGrpNames.find("option[value='"+ThisChartStandLines[0].GroupName+"']").attr("selected",true);
            $(this).find('#CustMDRStLinename_SD').hide();
            $(this).find('#CustMDRStLinegroupName_SD').show();
            groupNameChange(ThisChartStandLines);//绑定显示

            $("#btnEditGrp_SD").show();
        }else{
            $(this).find('#CustMDRStLinename_SD').show();
            $(this).find('#CustMDRStLinegroupName_SD').hide();
        }
...
Agi.Controls.CustomMDRChartView_SigmaSettingMenu=function(_Panel,_Control){
    var Me=_Control;
    var ItemContentPanel=$(_Panel);
    if($(_Panel).find(".content").length>0){
        ItemContentPanel=$(_Panel).find(".content");
    }
    ItemContentPanel.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-6', function () {
        ...
        $(this).find("#CustMDRSigmaValue").val(cp.ChartisigmaValue);
        var VisibleStateValue="1";
        if(!cp.ChartisigmaLineVisible){
            VisibleStateValue="0";
        }
        $(this).find("#CustMDRSigmaLineState").find("option[value='"+VisibleStateValue+"']").attr("selected",true);
        cp=null;
    });
}
//4.在View环境中显示西格玛判异菜单项
Agi.Controls.CustomMDRChartView_SigmaMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-3', function () {
        var THisChartSigmaRules=Me.Get("SigMDRules");//获取八项判异规则
        //1.八项规则保存
        $(this).find("#CustMDRSigMaRuleSave").unbind().bind("click",function(ev){
            var rule=[];
            var ruleitem=null;
            for(i=1;i<=8;i++)
            {
...
//5.特殊报警
Agi.Controls.CustomMDRChartView_SpecialAlarmMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-5', function () {
        //1.保存规则
...
//6.数据钻取设置
Agi.Controls.CustomMDRChartView_ExtractMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-7', function () {
        //0.默认选项隐藏
        $("#CustomMDRParCancel").hide();
        $("#CustomMDRNewParsTxt").width(0).hide();

        //1.添加参数
        $(this).find('#CustomMDRParAdd').click(function(){
            $(this).hide();
            $("#CustomMDRParCancel").show();
            $("#CustomMDRNewParsTxt").width(60).show();
            $("#CustomMDRParsNames").hide();
        });
        //2.取消添加
        $(this).find('#CustomMDRParCancel').click(function(){
            $("#CustomMDRParCancel").hide();
            $("#CustomMDRNewParsTxt").width(0).val("").hide();
            $("#CustomMDRParsNames").show();
            $("#CustomMDRParAdd").show();
        });
        //3.保存
        $(this).find('#CustomMDRParSave').click(function(){
            var SCSTxtVisibly=!$("#CustomMDRNewParsTxt").is(':hidden');
            var CusChartParsName="";
            var CusChartParsValue="";
            if(SCSTxtVisibly){
                //新增参数
                CusChartParsName=$("#CustomMDRNewParsTxt").val();
                CusChartParsValue=$("#CustomMDRParsValues").val();
            }else{
                //编辑参数
                CusChartParsName=$("#CustomMDRParsNames").val();
                CusChartParsValue=$("#CustomMDRParsValues").val();
            }
                ...
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    ...
                    if(SCSTxtVisibly){
                        if(ParIndex>-1){
                            ...
                            $("#CustomMDRParsNames").append("<option value='"+CusChartParsName+"'>"+CusChartParsName+"</option>");
                            ...
                            $("#CustomMDRParsNames").val(CusChartParsName);
                            //更新列表显示
                            Agi.Controls.CustomMDRChartParsListUpdate(ChartExtractConfig.Paras,"ParsListPanel");
        ...
        //4.删除
        $(this).find('#CustomMDRParDel').click(function(){
            var SCSTxtVisibly=!$("#CustomMDRNewParsTxt").is(':hidden');
            if(SCSTxtVisibly){
                AgiCommonDialogBox.Alert("请取消或完成新增，选择已存在的参数名称后再试!");
            }else{
                //编辑参数
               var CusChartParsName=$("#CustomMDRParsNames").val();
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    var ParIndex=-1;
                    for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                        if(ChartExtractConfig.Paras[i].Name==CusChartParsName){
                    ...
                    if(ParIndex>-1){
                        //移除对应的Option项
                        $("#CustomMDRParsNames option[value='"+ChartExtractConfig.Paras[ParIndex].Name+"']").remove(); //删除Select中Value=''的Option
        ...
        Agi.PageDataDrill.GetAllPageFilList(function(_result){
            if(_result.result && _result.result=="true"){
                ...
                if(_result.data!=null && _result.data.length>0){
                    ...
                    itemValue=null;
                    $("#CustMDRExtractURL").html(strCustSigChartExtractURLoptions);
                    if(ChartExtractConfig!=null && ChartExtractConfig.URL!=null){
                        $("#CustMDRExtractURL").find("[value='"+ChartExtractConfig.URL+"']").attr("selected","selected");
                    }
                }
            }
        })
        if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
            for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                $("#CustomMDRParsNames").append("<option value='"+ChartExtractConfig.Paras[i].Name+"'>"+ChartExtractConfig.Paras[i].Name+"</option>");
            }
            DefaultParNameColumn=ChartExtractConfig.Paras[0].Column;
            $("#CustomMDRParsNames").val(ChartExtractConfig.Paras[0].Name);

            //更新列表显示
            Agi.Controls.CustomMDRChartParsListUpdate(ChartExtractConfig.Paras,"ParsListPanel");
        }else{
            $("#CustomMDRParAdd").hide();
            $("#CustomMDRParCancel").show();
            $("#CustomMDRNewParsTxt").width(60).show();
            $("#CustomMDRParsNames").hide();
        }
        if(ChartEntityColumns!=null && ChartEntityColumns.length>0){
            for(var i=0;i<ChartEntityColumns.length;i++){
                $("#CustomMDRParsValues").append("<option value='"+ChartEntityColumns[i]+"'>"+ChartEntityColumns[i]+"</option>");
            }
            if(DefaultParNameColumn!=null){
                $("#CustomMDRParsValues").val(DefaultParNameColumn);
            }
        }
        //6.参数名称选中项更改
        $(this).find('#CustomMDRParsNames').change(function(){
            ...
            if(ThisParas!=null && ThisParas.length>0){
                for(var i=0;i<ThisParas.length;i++){
                    if(ThisParas[i].Name==StrThisSelValue){
                        $("#CustomMDRParsValues").val(ThisParas[i].Column);
                        break;
                    }
                }
            }
        ...
        //7.参数项&URL信息保存
        $(this).find('#CstMDRExtractOptionSave').click(function(){
            var ThisExtractConfig=Me.Get("ExtractConfig");//数据钻取配置信息保存
            ThisExtractConfig.URL=$("#CustMDRExtractURL").val();
...
//7.背景设置
Agi.Controls.CustomMDRChartView_BackgroundSetMenu=function(_Panel,_Control){
    var Me=_Control;
    //获取属性
    var cp = Me.Get('ChartProperty');
    ...
    ItemContentPanel.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-8', function () {
        $(this).find(".CustomMDRColorSty").spectrum({
            showInput: true,
            showPalette: true,
            palette: [
                ['black', 'white'],
        ...
        $("#CustomMDR_bgvalue").unbind().bind("click",function(){
        ...
        var ThisChartbgvalue=Agi.Controls.CustomMDRChartBackgroundValueGet(cp.ChartBackConfig.backgroundColor);
        $("#CustomMDR_bgvalue").css("background",ThisChartbgvalue.value.background).data('colorValue',ThisChartbgvalue);//设置默认项
        // backgroundColor//背景 ,Tolbtnbackground,//菜单按钮背景,TolbtnFontColor,//菜单按钮字体,TolTabbackground,//统计表格背景,TolTabFontColor//统计表格字体
        $("#CustomMDR_Tolbgvalue").spectrum("set",cp.ChartBackConfig.Tolbtnbackground);
        $("#CustomMDR_Tolbtncolor").spectrum("set",cp.ChartBackConfig.TolbtnFontColor);
        $("#CustomMDR_TolTabbg").spectrum("set",cp.ChartBackConfig.TolTabbackground);
        $("#CustomMDR_TolTabfontbg").spectrum("set",cp.ChartBackConfig.TolTabFontColor);

        $("#CustomMDR_BackgroundSave").unbind().bind("click",function(){
            var color=$("#CustomMDR_bgvalue").data('colorValue');
            ...
            var BackgroudObj={
                ...
                Tolbtnbackground:$("#CustomMDR_Tolbgvalue").val(),//统计信息按钮背景
                TolbtnFontColor:$("#CustomMDR_Tolbtncolor").val(),//统计信息按钮字体颜色
                TolTabbackground:$("#CustomMDR_TolTabbg").val(),//统计信息表格背景
                TolTabFontColor:$("#CustomMDR_TolTabfontbg").val()//统计信息表格字体颜色
            }
            Agi.Controls.CustomMDRChart.BackgroundApply(Me,BackgroudObj);//背景应用
            Me.RefreshByProPanelUp();//更新属性后，刷新图形显示
        })
    });
}
//9.标准差估计
Agi.Controls.CustomMDRChartView_RTypeMenu=function(_Panel,_Control){
    var Me=_Control;
    var cp = Me.Get('ChartProperty');
    //获取属性
    var ItemContentPanel=$(_Panel);
    ...
    ItemContentPanel.load('JS/Controls/CustomMDRChart/tabTemplates.html #tab-9', function () {
        $("#CustomMDR_RTypeSave").unbind().bind("click",function(){
            cp= Me.Get('ChartProperty');
            cp.optionVal=$('input:radio[name="CustomMDR_RTypeRadio"]:checked').val();
            Me.Set('ChartProperty',cp);
            Me.RefreshByProPanelUp();//更新属性后，刷新图形显示
        });
...
Agi.Controls.CustomMDRChartAlarmStatisticalTableShow=function(_AlarmStatisticaldata,_ApendToPanel,_BackGroundConfig){
    ...
    if($("#CustomMDRChartAlarmPanel").length>0){
        $("#CustomMDRChartAlarmPanel").html("<a id='btnTotalMenu' class='totalAlarmmenusty' >+统计信息</a></div>");
    }else{
        var CustomMDRChartAlarmPanelHTML=$("<div id='CustomMDRChartAlarmPanel' class='AlarmSty'></div>").appendTo($(_ApendToPanel));
        $("#CustomMDRChartAlarmPanel").css({"right":"8px","top":"8px"});
        CustomMDRChartAlarmPanelHTML.append("<a  id='btnTotalMenu' class='totalAlarmmenusty'>+统计信息</a>");
    }
    ...
    SubItemHTML+="</table>"
    $("#CustomMDRChartAlarmPanel").append(SubItemHTML);
    $("#CustomMDRChartAlarmPanel").find(".totalAlarmmenusty").css({
        "color":_BackGroundConfig.TolbtnFontColor,
        "background-color":_BackGroundConfig.Tolbtnbackground
    })
    $("#CustomMDRChartAlarmPanel").find("#TotalTable").css({
    ...
    $("#CustomMDRChartAlarmPanel").unbind().bind("dblclick",function(ev){
        ev.stopPropagation();    //  阻止事件冒泡
    });
...
//endregion
//region 编辑数据后显示对照图形
Agi.Controls.CustomMDRChartLastChartShow=function(_Controls){
    var Me=_Controls;
    if(Me.Get("LastChartImgIsExt")){
        Agi.Controls.CustomMDRChartLastChartShowMenu(Me,true);
    }else{
        var _canvas = [];
        ...
        html2canvas($("#"+ControlPanelID)[0],{
            onrendered:function(canvas, a){
                ...
                var ThisControlObj= Me.Get('chart');
               // $(ThisControlObj.renderTo).append("<div id='"+ControlPanelID+"_Img' class='MDRChartLastImgPanel'><img src='"+canvas.toDataURL('image/png')+"' style='width:100%;height:100%;'></div>");
                $("#"+ControlPanelID).append("<div id='"+ControlPanelID+"_Img' class='MDRChartLastImgPanel'><img src='"+canvas.toDataURL('image/png')+"' style='width:100%;height:100%;'></div>");
...
Agi.Controls.CustomMDRChartRefreshShowExe=function(_Controls){
    return function(){
        _Controls.RefreshByProPanelUp();
    };
}

Agi.Controls.CustomMDRChartLastChartShowMenu=function(_Control,_IsShow){
    var Me=_Control;
    var _ApendToPanel=Me.Get("HTMLElement");
    if(_IsShow){
        if($("#CustomMDRChartLastImgMenu").length>0){
        }else{
            var CustomMDRChartLastImgHTML=$("<div id='CustomMDRChartLastImgMenu' class='AlarmStyLasgImg' data-state='0'>查看原始图形</div>").appendTo($(_ApendToPanel));
            $("#CustomMDRChartLastImgMenu").css({"right":"78px","top":"8px"});
        }
        $("#CustomMDRChartLastImgMenu").show();
        $("#CustomMDRChartLastImgMenu").unbind().bind("click",function(ev){
            var ControlPanelID=$(Me.Get('HTMLElement'))[0].id;
        ...
        $("#CustomMDRChartLastImgMenu").hide();//隐藏
    }

}
//显示均值极差图数据钻取参数列表
Agi.Controls.CustomMDRChartParsListUpdate=function(_Paras,_ListPanelID){
    var StrListItems="";
    if(_Paras!=null && _Paras.length>0){
        for(var i=0;i<_Paras.length;i++){
            StrListItems+=" <div class='ParsListHeadSty'><div class='CustomMDRParsListCellSty'>"+_Paras[i].Name+"</div><div class='CustomMDRParsListCellSty'>"+_Paras[i].Column+"</div></div>";
        }
    }
...
//endregion
//region 背景设置
Agi.Controls.CustomMDRChartBackgroundValueGet=function(_oBackgroundObj){
...
Agi.Controls.CustomMDRChart.BackgroundApply=function(_ControlObj,_BackGroudObj){
...
Agi.Controls.CustomMDRChart.FindDataRowsByPoint=function(_checkDataSeries,_PointX){
//    var checkDataSeries = this.Get('CheckDataSeries');
    var PointRows=[];
...
Agi.Controls.CustomMDRChart.GetData=function(objValue,_Chartcp) {
    var minValue = objValue.minValue;
    var maxValue = objValue.maxValue;
    var objData = [];
...
//计算分组后的剩余数据行数
Agi.Controls.CustomMDRChart.GroupSizeDataSave=function(_GroupData,_nrow){
