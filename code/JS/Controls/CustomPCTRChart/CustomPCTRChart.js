...
Agi.Controls.CustomPCTRChart = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
...
//                    //endregion
                    for (var Name in ArrayBCName) {
                        for (var j = 0; j < EntityData.length; j++) {
                            if (EntityData[j][cp.SeriesGroupColumn] == ArrayBCName[Name]) {
                                DataArray.push(eval(EntityData[j][cp.SeriesDataColumn[0]]));
                            }
                        }
                    }
...
//                    DataArray.push(ArrayValue);
                ...
                var jsonData = { "action": "RCalp_Plot",
                    "dataArray": DataArray,
                ...
                if(cp.chart_Data_NRow.Type==1){
                    jsonData.nrow="";
                    jsonData.nrows=cp.chart_Data_NRow.ArrayValues;
                    DataArray=Agi.Controls.CustomPCTRChart.SeriesDataCal(DataArray,jsonData.nrows);
                }else{
                    DataArray=Agi.Controls.CustomPCTRChart.SeriesDataCal(DataArray,[jsonData.nrow]);
                    jsonData.nrow+="";
                }
                var jsonString = JSON.stringify(jsonData);
                Agi.DAL.ReadData({"MethodName":"RCalp_Plot","Paras": jsonString, "CallBackFunction": function (_result) {
                    //{"result":"true","Pbar":0.09525,"P_UCL":[0.11288,0.11263,0.11251],"P_LCL":[0.07762,0.07787,0.07799],"message":"获取成功！"}
                    if (_result && _result.result == "true") {
                        ...
                        checkDataSeries.push({
                                ...
                                minValue: _result.P_LCL,
                                middleValue: _result.Pbar,
                                maxValue: _result.P_UCL,
                                Isigma: null,
                                Data:DataArray
                            })
            ...
            if(cp.chart_Data_NRow.Type==1){
                ...
                try{
                    for(var i=0;i<FilterDataArray.length;i++){
                        cp.chart_Data_NRow.ArrayValues.push(eval(FilterDataArray[i][_dataLine.GroupSize.Value]));
                    }
                }catch(ex){}
            }else{
                cp.chart_Data_NRow.ArrayValues=[];
        ...
        GetDataSeries:function(){
            var Me=this;
            var DataSeries=Agi.Controls.CustomPCTRChartGetDataSries(Me.Get('ChartSeries'),true);
            return DataSeries;
        ...
        ShowALLPlotLineGroup:function(){
            ...
            if(_PlotLineGrps!=null && _PlotLineGrps.length>0){
                for(var i=0;i<_PlotLineGrps.length;i++){
                    newGroup=Agi.Controls.CustomPCTRChartFormatPlotGrpPars(Me.Get("Entity")[0],_PlotLineGrps[i]);//新增标准线组
                    if(newGroup!=null){
            ...
            var DataSeries=Agi.Controls.CustomPCTRChartGetDataSries(chartSeries);
            Agi.Controls.CustomPCTRChartPlotLineUpSeries(DataSeries,PlotLines);//更新Series 点数据
            ...
            if(WarnRuleValue!=null && WarnRuleValue.warnColumn!=null && FilterData!=null && FilterData.length>0){
                ...
                if(checkDataSeries!=null && checkDataSeries.length>0){
                    ...
                    for(var j=0;j<DataSeries[0].data.length;j++){
                        PointRows= Agi.Controls.CustomPCTRChart.FindDataRowsByPoint(checkDataSeries,DataSeries[0].data[j].x);
...
//            debugger;
        ...
        ApplySigmRule:function(_SeriesDataArray){
            ...
            if(SigmArray!=null && SigmArray.length>0){
                ...
                var ojb = new PCTRRclsSigmaFunc(_SeriesDataArray,SigmArray,CheckDataSries);
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            this.AttributeList = [];
            this.Set("Entity", []);
            this.Set("ControlType", "CustomPCTRChart");
            ...
            var ID = savedId ? savedId : "CustomPCTRChart" + Agi.Script.CreateControlGUID();
        ...
        SetChartProperty: function (_objCanvasAreaID) {//初始化定义图形控件属性结构，并预设值属性，_objCanvasAreaID：画布显示区域ID
            ...
            var chartProperty = {
                ...
                yAxis_Title_Text: 'P 控制图', //'单独值', //Y轴标题文字
        ...
        SetChartDataProperty: function () {//数据基本结构定义，在警戒范围中生成模拟数据
            ...
            {//数据结构生成
                ...
                for (var i = 0; i < checkDataSeries.length; i++) {
                    ...
                    Agi.Controls.CustomPCTRChart.MinMaxLineData(obj,dataMin,true);
                    ...
                    //判断并填充上下标准线的Series 点
                    Agi.Controls.CustomPCTRChart.MinMaxLineData(obj,dataMax,false);
        ...
        PointClick:function(_PointValue){
            ...
            var GridData={
                ...
                AlarmCells:[], //报警点信息
                AbnormalRows:Agi.Controls.CustomPCTRChart.FindDataRowsByPoint(checkDataSeries,PointRow)//所选的异常点
            };
...
//                {GroupName:"标准线2",MaxSize:1,MaxColor:'#04fe45',MaxValueType:0,MaxValue:29000,MinSize:1,MinColor:'#fbf806',MinValueType:0,MinValue:25800, MaxReviseSig:上限修正符号,MaxReviseValue:上限修正值,MinReviseSig:下限修正符号,MinReviseValue:下限修正值}]);//添加标准线
        ...
        CustomProPanelShow: function () {//显示自定义属性面板
            Agi.Controls.CustomPCTRChartProrityInit(this);
        },
        Destory: function () {
...
//            Agi.Controls.ControlDestoryByList(this);//移除控件,从列表中
        ...
        ControlAttributeChangeEvent: function (_obj, Key, _Value) {
            Agi.Controls.CustomPCTRChartAttributeChange(this, Key, _Value);
        },
        GetConfig: function () {
            var ProPerty = this.Get("ProPerty");
...
//                    alert(JSON.stringify(rowData));
        ...
        SPCViewStandLineMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomPCTRChartView_StandLinesMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，标准线
        SPCViewSigmaMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomPCTRChartView_SigmaMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，西格玛设置
        SPCViewDataExtractMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomPCTRChartView_ExtractMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，数据钻取
        SPCViewChartSetting:function(_Panel,_CallFun){
            ...
            $(_Panel).find(".content").height(200);
            Agi.Controls.CustomPCTRChartView_ChartSetMenu(_Panel, Me);
            _CallFun();
        },//图形选项
...
/*下拉列表控件参数更改处理方法*/
Agi.Controls.CustomPCTRChartAttributeChange = function (_ControlObj, Key, _Value) {
...
//拖拽控件至编辑页面时，初始控件的方法
Agi.Controls.InitCustomPCTRChart = function () {
    return new Agi.Controls.CustomPCTRChart();
}

Agi.Controls.CustomPCTRChartProrityInit = function (_BasicChart) {
    var Me = _BasicChart;
    //取得默认属性
    var cp = Me.Get('ChartProperty');
    ...
    //1.将属性项加入集合,有多少个属性就需要new对应个数的 Agi.Controls.Property.PropertyItem
    {
        ...
        Agi.Controls.CustomPCTRChartView_ChartSetMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomPCTRChartView_BackgroundSetMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomPCTRChartView_StandLinesMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomPCTRChartView_SigmaMenu(ItemContent,Me);
...
//        Agi.Controls.CustomPCTRChartView_ExtractMenu(ItemContent,Me);
...
//1.格式化标准线组的显示值信息
Agi.Controls.CustomPCTRChartFormatPlotGrpPars=function(_Entity,_PlotGrp){
    var PlotArray=null;
...
Agi.Controls.CustomPCTRChartPlotLineUpSeries=function(_SeriesArrar,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
//    var DataSeriesArray=Agi.Controls.CustomPCTRChartGetDataSries(_SeriesArrar);
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
            Agi.Controls.CustomPCTRChartPlotLineUpPoint(_SeriesArrar[i],_PlotLines);
        }
    }
}
Agi.Controls.CustomPCTRChartGetDataSries=function(_SeriesArrar,_IsDataSeries){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    var SeriesArray=[];
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
...
Agi.Controls.CustomPCTRChartPlotLineUpPoint=function(_Series,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    if(_Series!=null && _Series.data!=null && _Series.data.length>0){
        var PointColor=null;
        for(var i=0;i<_Series.data.length;i++){
            PointColor=Agi.Controls.CustomPCTRChartPlotLineGetPointColor(_Series.data[i].y,_PlotLines);
            if(PointColor!=null){
...
Agi.Controls.CustomPCTRChartPlotLineGetPointColor=function(_Value,_PlotLines){
    var PointColor=null;
...
//统计信息
Agi.Controls.CustomPCTRChartAlarmStatistical=function(_ALLSeries,_PlotLines){
    ...
    var _Series=Agi.Controls.CustomPCTRChartGetDataSries(_ALLSeries);//获得对应的的数据Series
    var _ScmSeries=Agi.Controls.CustomPCTRChartGetDataSries(_ALLSeries,false);//获得对应的的西格玛线Series
    ...
    if(_Series!=null || _ScmSeries!=null){
        ...
        for(var i=0;i<_Series.length;i++){
            for(var j=0;j<_Series[i].data.length;j++){
                Agi.Controls.CustomPCTRChartAlarmStatisticalByData(j,_Series[i].data[j].y,AlarmStatisticaldata,SumPointNum);//统计标准线所在比例信息
            }
        }
    }
...
Agi.Controls.CustomPCTRChartAlarmStatisticalByData=function(_index,_PointValue,_AlarmStatisticaldata,SumPointNum){
    //标准线超出点计算
...
//统计信息显示
Agi.Controls.CustomPCTRChartAlarmStatisticalPanelShow=function(_AlarmStatisticaldata,_ApendToPanel){
    if($("#CustomPCTRChartAlarmPanel").length>0){
        $("#CustomPCTRChartAlarmPanel").html("");
    }else{
        var CustomPCTRChartAlarmPanelHTML=$("<div id='CustomPCTRChartAlarmPanel' class='AlarmSty'></div>").appendTo($(_ApendToPanel));
        $("#CustomPCTRChartAlarmPanel").css({"right":"8px","top":"8px"});
    }
    var SubItemHTML="";
    ...
    $("#CustomPCTRChartAlarmPanel").append(SubItemHTML);
}
...
//2.图形选项
Agi.Controls.CustomPCTRChartView_ChartSetMenu=function(_Panel,_Control){
    var Me=_Control;
    //3.1.初始化数据准备
    var ChartEntity=Me.Get("Entity")[0];
    ...
    ItemContentPanel.load('JS/Controls/CustomPCTRChart/tabTemplates.html #tab-1', function () {
        $(this).find("#CustPCTRdataColor").spectrum({
            showInput: true,
            showPalette: true,
            palette: [
                ['black', 'white'],
        ...
        $(this).find("#CustPCTRmarkerColor").spectrum({
            showInput: true,
            showPalette: true,
            palette: [
                ['black', 'white'],
        ...
        $("input[name='PCTR_SubGrp']").unbind().bind("click",function(ev){
            var ThisSelectedType=$("input[name=PCTR_SubGrp]:checked").attr("value");
            if(ThisSelectedType==0){
                $("#PCTR_SubGrpColumns").hide();
                $("#XMRChart_RGroupSize").show();
            }else{
                $("#PCTR_SubGrpColumns").show();
                $("#XMRChart_RGroupSize").hide();
            }
        });

        //默认选中
        $(this).find("#CustPCTRdataColor").spectrum("set","#d2d2d2");
        $(this).find("#CustPCTRmarkerColor").spectrum("set","#4572A7");
        if(cp.series_DataColor!=null && cp.series_PointMarkerColor!=null){
            $(this).find("#CustPCTRdataColor").spectrum("set",cp.series_DataColor);
            $(this).find('#CustPCTRmarkerColor').spectrum("set",cp.series_PointMarkerColor);
        }
        //分组列和数据列绑定
        var GroupColumns=$(this).find('#CustPCTRGrpColumn');
        var DataColumns=$(this).find('#CustPCTRDataColumn');
        var SubGroupColumns=$(this).find('#PCTR_SubGrpColumns');
        GroupColumns.empty();
        DataColumns.empty();
        ...
        if(cp.SeriesGroupColumn && cp.SeriesDataColumn!=null){
            ...
            if(cp.SeriesGroupColumn==null){
                cp.SeriesGroupColumn=$($("#CustPCTRGrpColumn").children()[0]).val();
            }
            if(cp.SeriesDataColumn==null){
                cp.SeriesDataColumn=$($("#CustPCTRDataColumn").children()[0]).val();
            }
        }
        ...
        if(cp.chart_Data_NRow!=null && cp.chart_Data_NRow.Type==1){
            ...
            $("#PCTR_SubGrp_1").attr("checked","checked");
            $("#XMRChart_RGroupSize").hide();
            $("#PCTR_SubGrpColumns").show();
            ...
            $("#PCTR_SubGrp_0").attr("checked","checked");
            SubGroupColumns.find('option:eq(0)').attr('selected','selected');
        }

        var CustPCTRChartSeriesItems= $(this).find("#CustPCTRSeries").empty();
        if(ChartDataSeries!=null && ChartDataSeries.length>0){
            for(var i=0;i<ChartDataSeries.length;i++) {
                ...
                CustPCTRChartSeriesItems.append(option);
            }
            CustPCTRChartSeriesItems.find("option[value='"+ChartDataSeries[0].name+"']").attr("selected","selected");
        }

        //保存处理
        $(this).find("#CustPCTRSeriesSave").unbind().bind("click",function(ev){
            var CustPCTRChartSeriesObj={
                SeriesName:$("#CustPCTRSeries").val(),
                GroupColumn:$("#CustPCTRGrpColumn").val(),
                DataColumn:$("#CustPCTRDataColumn").val(),
                SeriesColor:$("#CustPCTRdataColor").val(),
                SeriesMarkerColor:$("#CustPCTRmarkerColor").val(),
                GroupSize:{
                    Type:0,//类型
                    Value:$("#XMRChart_RGroupSize").val()
                }//分组大小
            }
            if($("#PCTR_SubGrp_0").is(":checked")){}else{
                CustPCTRChartSeriesObj.GroupSize.Type=1;
                CustPCTRChartSeriesObj.GroupSize.Value=$("#PCTR_SubGrpColumns").val();
            }
            Me.UpSeriesBindInfo(CustPCTRChartSeriesObj);//更新Series 设置
...
Agi.Controls.CustomPCTRChartView_StandLinesMenu=function(_Panel,_Control){
    var Me=_Control;
    //3.1.初始化数据准备
    var ChartEntity=Me.Get("Entity")[0];
    var cp=Me.Get("ChartProperty");//获取属性
    ...
    //3.2.加载标准线设置菜单
    ItemContentPanel.load('JS/Controls/CustomPCTRChart/tabTemplates.html #tab-2', function () {
        //0.控件当前值获取
        ...
        $(this).find("#btnSaveGrp").click(function(){
            ...
            var line={
                GroupName:$("#CustPCTRStLinename").attr('value'),
            ...
            cp.ChartisigmaValue=$("#PCTRChart_BZSize").val();
        ...
        if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
            $("#PCTRChart_BZSize").val(cp.ChartisigmaValue);
...
Agi.Controls.CustomPCTRChartView_SigmaMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomPCTRChart/tabTemplates.html #tab-3', function () {
        var THisChartSigmaRules=Me.Get("Sigmrules");//获取八项判异规则
        //1.八项规则保存
        $(this).find("#CustPCTRSigMaRuleSave").unbind().bind("click",function(ev){
            var rule=[];
            var ruleitem=null;
            for(i=1;i<=4;i++)
            {
...
//endreigon
//6.数据钻取设置
Agi.Controls.CustomPCTRChartView_ExtractMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('JS/Controls/CustomPCTRChart/tabTemplates.html #tab-4', function () {
        //0.默认选项隐藏
        $("#CustomPCTRParCancel").hide();
        $("#CustomPCTRNewParsTxt").width(0).hide();

        //1.添加参数
        $(this).find('#CustomPCTRParAdd').click(function(){
            $(this).hide();
            $("#CustomPCTRParCancel").show();
            $("#CustomPCTRNewParsTxt").width(60).show();
            $("#CustomPCTRParsNames").hide();
        });
        //2.取消添加
        $(this).find('#CustomPCTRParCancel').click(function(){
            $("#CustomPCTRParCancel").hide();
            $("#CustomPCTRNewParsTxt").width(0).val("").hide();
            $("#CustomPCTRParsNames").show();
            $("#CustomPCTRParAdd").show();
        });
        //3.保存
        $(this).find('#CustomPCTRParSave').click(function(){
            var SCSTxtVisibly=!$("#CustomPCTRNewParsTxt").is(':hidden');
            var CusChartParsName="";
            var CusChartParsValue="";
            if(SCSTxtVisibly){
                //新增参数
                CusChartParsName=$("#CustomPCTRNewParsTxt").val();
                CusChartParsValue=$("#CustomPCTRParsValues").val();
            }else{
                //编辑参数
                CusChartParsName=$("#CustomPCTRParsNames").val();
                CusChartParsValue=$("#CustomPCTRParsValues").val();
            }
                ...
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    ...
                    if(SCSTxtVisibly){
                        if(ParIndex>-1){
                            ...
                            $("#CustomPCTRParsNames").append("<option value='"+CusChartParsName+"'>"+CusChartParsName+"</option>");
                            ...
                            $("#CustomPCTRParsNames").val(CusChartParsName);
                            //更新列表显示
                            Agi.Controls.CustomPCTRChartParsListUpdate(ChartExtractConfig.Paras,"ParsListPanel");
        ...
        //4.删除
        $(this).find('#CustomPCTRParDel').click(function(){
            var SCSTxtVisibly=!$("#CustomPCTRNewParsTxt").is(':hidden');
            if(SCSTxtVisibly){
                AgiCommonDialogBox.Alert("请取消或完成新增，选择已存在的参数名称后再试!");
            }else{
                //编辑参数
               var CusChartParsName=$("#CustomPCTRParsNames").val();
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    var ParIndex=-1;
                    for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                        if(ChartExtractConfig.Paras[i].Name==CusChartParsName){
                    ...
                    if(ParIndex>-1){
                        //移除对应的Option项
                        $("#CustomPCTRParsNames option[value='"+ChartExtractConfig.Paras[ParIndex].Name+"']").remove(); //删除Select中Value=''的Option
        ...
        Agi.PageDataDrill.GetAllPageFilList(function(_result){
            if(_result.result && _result.result=="true"){
                ...
                if(_result.data!=null && _result.data.length>0){
                    ...
                    itemValue=null;
                    $("#CustPCTRExtractURL").html(strCustSigChartExtractURLoptions);
                    if(ChartExtractConfig!=null && ChartExtractConfig.URL!=null){
                        $("#CustPCTRExtractURL").find("[value='"+ChartExtractConfig.URL+"']").attr("selected","selected");
                    }
                }
            }
        })
        if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
            for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                $("#CustomPCTRParsNames").append("<option value='"+ChartExtractConfig.Paras[i].Name+"'>"+ChartExtractConfig.Paras[i].Name+"</option>");
            }
            DefaultParNameColumn=ChartExtractConfig.Paras[0].Column;
            $("#CustomPCTRParsNames").val(ChartExtractConfig.Paras[0].Name);

            //更新列表显示
            Agi.Controls.CustomPCTRChartParsListUpdate(ChartExtractConfig.Paras,"ParsListPanel");
        }else{
            $("#CustomPCTRParAdd").hide();
            $("#CustomPCTRParCancel").show();
            $("#CustomPCTRNewParsTxt").width(60).show();
            $("#CustomPCTRParsNames").hide();
        }
        if(ChartEntityColumns!=null && ChartEntityColumns.length>0){
            for(var i=0;i<ChartEntityColumns.length;i++){
                $("#CustomPCTRParsValues").append("<option value='"+ChartEntityColumns[i]+"'>"+ChartEntityColumns[i]+"</option>");
            }
            if(DefaultParNameColumn!=null){
                $("#CustomPCTRParsValues").val(DefaultParNameColumn);
            }
        }
        //6.参数名称选中项更改
        $(this).find('#CustomPCTRParsNames').change(function(){
            ...
            if(ThisParas!=null && ThisParas.length>0){
                for(var i=0;i<ThisParas.length;i++){
                    if(ThisParas[i].Name==StrThisSelValue){
                        $("#CustomPCTRParsValues").val(ThisParas[i].Column);
                        break;
                    }
                }
            }
        ...
        //7.参数项&URL信息保存
        $(this).find('#CstPCTRExtractOptionSave').click(function(){
            var ThisExtractConfig=Me.Get("ExtractConfig");//数据钻取配置信息保存
            ThisExtractConfig.URL=$("#CustPCTRExtractURL").val();
...
//7.背景设置
Agi.Controls.CustomPCTRChartView_BackgroundSetMenu=function(_Panel,_Control){
    var Me=_Control;
    //获取属性
    var cp = Me.Get('ChartProperty');
    ...
    ItemContentPanel.load('JS/Controls/CustomPCTRChart/tabTemplates.html #tab-5', function () {
        ...
        $("#CustomPCTR_bgvalue").unbind().bind("click",function(){
        ...
        var ThisChartbgvalue=Agi.Controls.CustomPCTRChartBackgroundValueGet(cp.ChartBackConfig.backgroundColor);
        $("#CustomPCTR_bgvalue").css("background",ThisChartbgvalue.value.background).data('colorValue',ThisChartbgvalue);//设置默认项
        // backgroundColor//背景 ,Tolbtnbackground,//菜单按钮背景,TolbtnFontColor,//菜单按钮字体,TolTabbackground,//统计表格背景,TolTabFontColor//统计表格字体
        $("#CustomPCTR_Tolbgvalue").spectrum("set",cp.ChartBackConfig.Tolbtnbackground);
        $("#CustomPCTR_Tolbtncolor").spectrum("set",cp.ChartBackConfig.TolbtnFontColor);
        $("#CustomPCTR_TolTabbg").spectrum("set",cp.ChartBackConfig.TolTabbackground);
        $("#CustomPCTR_TolTabfontbg").spectrum("set",cp.ChartBackConfig.TolTabFontColor);

        $("#CustomPCTR_BackgroundSave").unbind().bind("click",function(){
            var color=$("#CustomPCTR_bgvalue").data('colorValue');
            ...
            Agi.Controls.CustomPCTRChart.BackgroundApply(Me,BackgroudObj);//背景应用
            Me.RefreshByProPanelUp();//更新属性后，刷新图形显示
        })
    });
}
...
//region 编辑数据
//显示单值图数据钻取参数列表
Agi.Controls.CustomPCTRChartParsListUpdate=function(_Paras,_ListPanelID){
    var StrListItems="";
    if(_Paras!=null && _Paras.length>0){
        for(var i=0;i<_Paras.length;i++){
            StrListItems+=" <div class='ParsListHeadSty'><div class='CustomPCTRParsListCellSty'>"+_Paras[i].Name+"</div><div class='CustomPCTRParsListCellSty'>"+_Paras[i].Column+"</div></div>";
        }
    }
...
//endregion
//region 背景设置
Agi.Controls.CustomPCTRChartBackgroundValueGet=function(_oBackgroundObj){
...
Agi.Controls.CustomPCTRChart.BackgroundApply=function(_ControlObj,_BackGroudObj){
...
Agi.Controls.CustomPCTRChart.FindDataRowsByPoint=function(_checkDataSeries,_PointX){
//    var checkDataSeries = this.Get('CheckDataSeries');
    var PointRows=[];
...
Agi.Controls.CustomPCTRChart.MinMaxLineData=function(_ThischeckSeriesObj,_MinMaxData,_IsMin){
    if(_ThischeckSeriesObj.minValue.length>1){
        for(var i=0;i<_ThischeckSeriesObj.Data.length;i++){
            //{ x: x, y: y, marker: { fillColor: cp.yAxis_PlotLines_MaxMarkerColor} };
            if(_IsMin){
                if(_ThischeckSeriesObj.Data[i].x>0){
                    _MinMaxData.push([_ThischeckSeriesObj.Data[i].x-0.5, _ThischeckSeriesObj.minValue[i]]);
                    _MinMaxData.push([_ThischeckSeriesObj.Data[i].x+0.5, _ThischeckSeriesObj.minValue[i]]);
                }else{
                    _MinMaxData.push([0, _ThischeckSeriesObj.minValue[i]]);
                    _MinMaxData.push([0.5, _ThischeckSeriesObj.minValue[i]]);
                }
            }else{
                if(_ThischeckSeriesObj.Data[i].x>0){
                    _MinMaxData.push([_ThischeckSeriesObj.Data[i].x-0.5, _ThischeckSeriesObj.maxValue[i]]);
                    _MinMaxData.push([_ThischeckSeriesObj.Data[i].x+0.5, _ThischeckSeriesObj.maxValue[i]]);
                }else{
                    _MinMaxData.push([0, _ThischeckSeriesObj.maxValue[i]]);
                    _MinMaxData.push([0.5, _ThischeckSeriesObj.maxValue[i]]);
                }
            }
        }
    }else{
...
//曲线Series 计算
Agi.Controls.CustomPCTRChart.SeriesDataCal=function(_DataArray,_GroupValue){
    var TempDataArray=[];
