        ...
        ShowTextBox:function(self){
            ...
            document.getElementById(self.shell.BasicID).getElementsByTagName("a").item(0).onclick=function(){
                ...
                $("#HiddenText"+self.shell.BasicID).css({"position":"absolute","margin-left":"0px","margin-top":"0px",
                    ...
                    "height":eval(eval($("#"+self.shell.ID).css("height").replace("px",""))-2)+"px","z-index":eval($("#"+self.shell.ID).css("z-index"))+1});
                ...
                document.getElementById("BottomRightCenterContentDiv").onclick=function(){
...
Agi.Controls.DropDownListProrityInit = function (dropDownControl) {
    ...
    //  2014-02-26   coke  是否筛选
    $(['是否筛选']).each(function (i, label) {
        BindHTMLTable.append("<tr>" +
            "<td class='DropDownListProTabtd0'>" + label + "：</td>" +
            "<td class='DropDownListProTabtd1'  colspan='3'> <input type=\"checkbox\"  title=" + label + " data-field=" + label + " placeholder='' class='input' ></td>" +
            "<td></td><td></td>" +
            "</tr>");
    });

    //  2014-02-26   coke  是否筛选
    bindHTML.find('input').append($(options)).bind('change', { sels: bindHTML.find('input') }, function (e) {
        $(e.data.sels).each(function (i, sel) {
            var f = $(sel).data('field');
            if (f == '是否筛选') {
                BasicProperty.IsNoFilter = $(sel).attr("checked")=="checked"?true:false;
            }
        });
        dropDownControl.Set('BasicProperty', BasicProperty);
    }).each(function (i, sel) {
            var f = $(sel).data('field');
            if (f == '是否筛选') {
                $(sel).attr("checked", BasicProperty.IsNoFilter);
            }
        });
