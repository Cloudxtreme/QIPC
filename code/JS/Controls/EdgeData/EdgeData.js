...
Agi.Controls.EdgeData = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
        ...
        //设置边降数据上下限
        SetEdgeDataYAxisRange:function(setting,ischecked){
        	var chart=this.chart;
        	var cp = this.Get('ChartProperty');
        	cp.chart_max_line=setting.max;
        	cp.chart_min_line=setting.min;
        	cp.chart_fixed_range=ischecked;
...
//                                    cp.series_MiddlePlotLine_Enabled = true;
        ...
        SetChartProperty: function (_objCanvasAreaID) {//初始化定义图形控件属性结构，并预设值属性，_objCanvasAreaID：画布显示区域ID
            ...
            var chartProperty = {
                ...
                chart_max_line:null,//chart的上显示边界值
                chart_min_line:null,//chart的下显示边界值
                chart_std_top_color:'red',//chart的标准线上限颜色
                chart_std_bottom_color:'orange',//chart的标准线下限颜色
                chart_std_top_visible:true,//chart的标准线上限是否显示
                chart_std_bottom_visible:true,//chart的标准线下限是否显示
                chart_std_top_val:30,//chart的标准线上限值
                chart_std_bottom_val:-10,//chart的标准线下限值
                chart_std_top_linewidth:2,//chart的标准线上限线宽
                chart_std_bottom_linewidth:2,//chart的标准线下限线宽
                chart_std_over_color:'red',//超出标准线上限的点颜色
                chart_std_below_color:'orange',//低于标准线下线的点颜色
                chart_std_regular_color:'green'//'#2F7ED8'//在标准线设定范围内的点颜色
            };
        ...
        InitEdgeData: function () {//初始化图形控件
            ...
            if (cp.chart_RenderTo) {
                var edgedata=new EdgeData({
            		...
            		stdovercolor:cp.chart_std_over_color,//超出标准线上限的点颜色
                    stdbelowcolor:cp.chart_std_below_color,//低于标准线下线的点颜色
                    stdregularcolor:cp.chart_std_regular_color//在标准线设定范围内的点颜色
            	});
...
Agi.Controls.EdgeDataProrityInit = function (_BasicChart) {
    ...
    var ThisProItems = [];
    {
        ...
        //region 4.2.标准线
        ItemContent=$("<div id='EdgeDataChartStdLine'></div>");
...
Agi.Controls.EdgeDataRangeSetter=function(_Panel,_Control){
	var Me=_Control,ItemContent=_Panel;
	ItemContent.load('JS/Controls/EdgeData/tabTemplates.html #tab-1', function () {
    	...
    	function saveRangeSet(ischecked){
    		...
    		Me.SetEdgeDataYAxisRange({'max':max,'min':min},ischecked);
    	}
    });
}
//设置标准线配置界面
