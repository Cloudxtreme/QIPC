...
Agi.Controls.NewCheckBox = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
        ...
        ReadData: function (et) {
            var self = this;
            var BasicProperty = self.Get('BasicProperty');
            //20130524 倪飘 解决bug，新复选框，当由自定义数据切换为实体数据时，给出提示信息"若添加实体数据将清空自定义数据，是否切换？"
            if (BasicProperty.IsUseEntity === false) {
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            //20130515 倪飘 解决bug，组态环境中拖入新复选框控件以后拖入容器框控件，容器框控件会覆盖其他控件（容器框控件添加背景色以后能够看到效果）
...
//        },
        PostionChange: function (_Postion) {
            if (_Postion != null && _Postion.Left != null && _Postion.Top != null && _Postion.Right != null && _Postion.Bottom != null) {
                ...
                //                var width = parseInt(parseInt($("#" + id).width()) / 15);
                //                ThisHTMLElement.find(".CheckBoxForm label").css("margin-right", width+"px");
                ////                ThisHTMLElement.find(".CheckBoxForm label").css();
                this.Set("Position", _ThisPosition);
                PagePars = _ThisPosition = null;
            }
        },
        ...
        CreateControl: function (_Config, _Target) {
            ...
            if (_Config != null) {
                ...
                if (_Target != null && _Target != "") {
                    ...
                    //解决新复选框，拖入共享数据源数据，选择几个选项，点击预览，预览页面中复选框选项显示未选择状态
...
Agi.Controls.NewCheckBoxProrityInit = function (NewCheckBoxControl) {
    ...
    ItemContent.append("</tr>");
    ItemContent.append("<tr>");
    ItemContent.append("<td class='NewCheckBox_prortityPanelTabletd0'>左上圆角：</td><td class='NewCheckBox_prortityPanelTabletd2'><input id='CheckLTR' type='number' max='50' min='0' ></td>");
    ItemContent.append("<td class='NewCheckBox_prortityPanelTabletd0'>右上圆角：</td><td class='NewCheckBox_prortityPanelTabletd2'><input  id='CheckRTR' type='number' max='50' min='0' ></td>");
    ItemContent.append("</tr>");
    ItemContent.append("<tr>");
    ItemContent.append("<td class='NewCheckBox_prortityPanelTabletd0'>左下圆角：</td><td class='NewCheckBox_prortityPanelTabletd2'><input  id='CheckLBR' type='number' max='50' min='0'></td>");
    ItemContent.append("<td class='NewCheckBox_prortityPanelTabletd0'>右下圆角：</td><td class='NewCheckBox_prortityPanelTabletd2'><input id='CheckRBR' type='number' max='50' min='0'></td>");
    ItemContent.append("</tr>");
    ItemContent.append("<tr>");
    ...
    ItemContent.append("</tr>");
    ItemContent.append("<tr>");
    ItemContent.append("<td class='NewCheckBox_prortityPanelTabletd0'>边框大小：</td><td class='NewCheckBox_prortityPanelTabletd2'><input id='CheckBorderSize' type='number' min='0' max='10'></td>");
    ...
    $("#CheckBorderSize").change(function () {
        ...
        if (val.trim() === "") {
            $(this).val(BasicProperty.BorderSize.substr(0, BasicProperty.BorderSize.indexOf('p')));
