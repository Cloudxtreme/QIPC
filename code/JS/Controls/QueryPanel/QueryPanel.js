...
/*添加 Agi.Controls命名空间*/
Agi.Controls.QueryPanel = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
        ...
        /** 收起/打开功能START*/
        InitOCButton:function(parent){
        	var thisobj=this;
        	var btncon=$('<div>')
        		.css({'position':'absolute','top':'0','left':'0','border':'1px solid gray',
        			'background-color':'lavender','width':'100%'})
        		.prop('title','收起').html('&nbsp;&nbsp;查询参数');
        	var btndiv=$('<div>').addClass('ui-icon ui-icon-arrowthick-1-n').css('float','right');
        	btncon.append(btndiv);
        	parent.append(btncon);
        	if(!thisobj.occache){
        		thisobj.occache={
            			'isopen':true,
            			'theight':btncon.parent().height(),
            			'cheight':0,
            			'attachment':[]
            	};
        	}
        	...
        	btncon.click(function(evt){
        		thisobj.SwitchOCStatus();
        		evt.stopPropagation();
        		evt.preventDefault();
        	});
        	btncon.dblclick(function(evt){
        		evt.stopPropagation();
        		evt.preventDefault();
        	});
        	/*btncon.bind(
    				'mouseenter mouseout', function(evt) {
    					console.log(new Date()+evt.type);
    					var opacity = 0;
        ...
        SwitchOCStatus:function(){
        	var thisobj=this;
        	var con=thisobj.occantainer.con,div=thisobj.occantainer.div;
        	if(thisobj.occache.isopen){
        		thisobj.QueryAreaClose();
        	}else{
        		thisobj.QueryAreaOpen();
        	}
        	thisobj.occache.cheight=con.height();
        },
        QueryAreaOpen:function(){
        	var thisobj=this;
        	var con=thisobj.occantainer.con,div=thisobj.occantainer.div;
        	con.parent().stop(true).animate({'height':thisobj.occache.theight},500);
    		thisobj.QueryIconChange(true);
    		thisobj.InformContactQCWithOCAction(true);
        	thisobj.occache.isopen=true;
        },
        QueryAreaClose:function(){
        	var thisobj=this;
        	var con=thisobj.occantainer.con,div=thisobj.occantainer.div;
        	con.parent().stop(true).animate({'height':con.outerHeight()},500);
        	thisobj.QueryIconChange(false);
        	thisobj.InformContactQCWithOCAction(false);
        	thisobj.occache.isopen=false;
        },
        QueryIconChange:function(toOpen){
        	var thisobj=this;
        	var con=thisobj.occantainer.con,div=thisobj.occantainer.div;
        	if(toOpen){
        		div.removeClass('ui-icon-arrowthick-1-s').addClass('ui-icon-arrowthick-1-n');
        		con.prop('title','收起');
        	}else{
        		div.removeClass('ui-icon-arrowthick-1-n').addClass('ui-icon-arrowthick-1-s');
        		con.prop('title','打开');
        	}
        },
        InformContactQCWithOCAction:function(isOpen){
        	var thisobj=this,ats=thisobj.occache.attachment;
        	if(ats.length<=0){
        		return;
        	}
        	
    		for(var i=0;i<ats.length;i++){
    			var attaControl=Agi.Controls.FindControl(ats[i]);
    			if(!attaControl) continue;
    			/*if(isOpen){
    				attaControl.Get('ProPerty').BasciObj.parent().css('visibility','visible')//show();
        		}else{
        			attaControl.Get('ProPerty').BasciObj.parent().css('visibility','hidden')//hide();
        		}*/
    			thisobj.SetQueryControlVisibility(attaControl,isOpen);
        	}
        },
        SetQueryControlVisibility:function(control,visible){
        	$('#Panel_'+control.Get('ProPerty').ID).css('visibility',visible?'visible':'hidden');
        },
        ResetOCByPosition:function(height){
        	var thisobj=this;
        	var oldheight=thisobj.occache.cheight;
        	if(Math.abs(oldheight-height)>=5){//消除移动的时候产生的高度误差....竟然有5像素之多....
        		thisobj.occache.isopen=true;
            	thisobj.QueryIconChange(true);
            	thisobj.occache.theight=height;
        	}
        },
        ContactWithQueryControl:function(action,ids){
        	var thisobj=this,ats=thisobj.occache.attachment;
        	if(action=='refresh'){
        		thisobj.occache.attachment=[];
        		for(var i=0;i<ids.length;i++){
        			thisobj.occache.attachment.push(ids[i]);
        		}
        	}else if(action=='add'){
        		if($.inArray(ids,ats)==-1){
        			ats.push(ids);
        		}
        	}else if(action=='del'){
        		var idx=$.inArray(ids,ats);
        		if(idx!=-1){
        			ats.splice(idx,1);
        		}
        	}
        ...
        //当位置发生变化时,通知所有关联控件联动
        PositionLinkage:function(oldPos,newPos){
        	var thisobj=this,ats=thisobj.occache.attachment;
        	if(ats.length<=0){
        		return;
        	}
        	//只有left和top改变时才需要通知其他控件改变
        	if(oldPos.Left!=newPos.Left || oldPos.Top!=newPos.Top){
        		var leftAddtion=newPos.Left-oldPos.Left;
        		var topAddtion=newPos.Top-oldPos.Top;
        		for(var i=0;i<ats.length;i++){
        			var atsObj=Agi.Controls.FindControl(ats[i]);
        			if(!atsObj) continue;
        			var atsObjPos=atsObj.Get('Position');
        			atsObjPos.Left=parseFloat(atsObjPos.Left)+leftAddtion;
        			atsObjPos.Top=parseFloat(atsObjPos.Top)+topAddtion;
        			atsObjPos.Right=parseFloat(atsObjPos.Right)-leftAddtion;
        			atsObjPos.Bottom=parseFloat(atsObjPos.Bottom)-topAddtion;
        			atsObj.Refresh();
        		}
        	}
        },
        ...
        Init: function (_Target, _ShowPosition) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            this.AttributeList = [];
            //this.Set("Entity", []);
            this.Set("ControlType", "QueryPanel");
            var ID = "Panel" + Agi.Script.CreateControlGUID();
            var ThisProPerty = {
                ID: ID,
                BasciObj: $("<div id='" + ID + "' class='PanelSty agi-panel'></div>"),
                IsUseStyle: false
            };
            this.shell = new Agi.Controls.Shell({
                ...
                width: 200,
                height: 60,
                divPanel: ThisProPerty.BasciObj
            });
            this.shell.setTitle(ID);
            ...
            this.InitOCButton(ThisProPerty.BasciObj);
...
//        },
        PostionChange: function (_Postion) {
            ...
            this.PositionLinkage(oldPosition,this.Get('Position'));
        },
        ...
        ControlAttributeChangeEvent: function (_obj, Key, _Value) {
            ...
            if (Key == "Position") {
                if (layoutManagement.property.type == 1) {
                    ...
                    var elementHeight=PagePars.Height - ((parseFloat(_Value.Top) + parseFloat(_Value.Bottom)) * PagePars.Height);
                    var oldheight=ThisHTMLElement.height();
                    ThisHTMLElement.height(elementHeight);
                	...
                	this.ResetOCByPosition(elementHeight);
                }
            }
            if (Key == "PanelFilter") {
                if (layoutManagement.property.type == 1) {
                    ...
                    if(_Value.FilterBgImage.indexOf("SourceManager/")>-1){
        ...
        GetConfig: function () {
            ...
            PanelContorl.Control.OCCache=this.occache;
        ...
        CreateControl: function (_Config, _Target) {
            ...
            if (_Config != null) {
                if (_Target != null && _Target != "") {
                    ...
                    this.occache=_Config.OCCache;
                    this.shell.setTitle(_Config.ControlID);
                    this.Set("ProPerty", ThisProPerty);
                    this.Set("HTMLElement", ThisProPerty.BasciObj[0]);
        ...
        ChangeTheme: function (_themeName) {
            ...
            Agi.Controls.QueryPanel.OptionsAppSty(PanelStyleValue, Me);
            //5.控件刷新显示
            //    Me.Refresh();//刷新显示
 ...
 * _Options:控件相关参数信息
 * */
Agi.Controls.QueryPanel.OptionsAppSty = function (_StyConfig, _Panel) {
    if (_StyConfig != null) {
...
//拖拽控件至编辑页面时，初始控件的方法
Agi.Controls.InitQueryPanel = function () {
    return new Agi.Controls.QueryPanel();
}
//var _Panel;
