            ...
            if (_EntityInfo != undefined) {
                ...
                if (!_EntityInfo.IsShareEntity) {
                    Agi.Utility.RequestData2(_EntityInfo, function (d) {
                        ...
                        //默认数据集第一列为时间，其他列为对应数值
                        var colums = {
                            dtKeyName:null, //时间列列名
                            valKeyNames:[]  //其他列列名
                        }
                        if (d.Columns) {
                            colums.dtKeyName= d.Columns[0];
                            for(var i=1;i< d.Columns.length;i++) {
                                colums.valKeyNames.push(d.Columns[i]);
                            }
                        } else {
                            var _d = d.Data[0];
                            colums.dtKeyName= _d[0];
                            for(var i=1;i< _d.length;i++) {
                                colums.valKeyNames.push(_d[i]);
                            }
                        }
                        ...
                        //拼series数据
                        var series=[];
                        for (var _valKeyNameIndex in colums.valKeyNames) {
                            var sUnit ={
                                name:colums.valKeyNames[_valKeyNameIndex],
                                data:[],
                                tooltip:{
                                    valueDecimals: 2
                                }
                            };
                            for (var _dataIndex in d.Data) {
                                var dt = d.Data[_dataIndex];
                                sUnit.data.push([parseFloat(dt[colums.dtKeyName]),parseFloat(dt[colums.valKeyNames[_valKeyNameIndex]])]);
                            }
                            series.push(sUnit);
                        }
                        SeqChartDataProperty.series= series;
                        Me.Set("SeqChartDataProperty", SeqChartDataProperty);
                    });
                }
            }
        },
...
//                //region 直方图
        ...
        Init: function (_Target, _ShowPosition) {
            ...
            this.AttributeList = [];
            this.Set("Entity", []);
            this.Set("ControlType", "SeqChart");
            var ID = "SeqChart" + Agi.Script.CreateControlGUID();
            var HTMLElementPanel = $("<div recivedata='true' id='Panel_" + ID +
                "' class='PanelSty PCChartPanelSty'><div id='Panel_chart_" + ID +
                "' class='pcchartpanel' style='border:1px solid green;float:left;height:100%;'></div><div id='Panel_MenuObj_" + ID +
                "' class='pcchartmenu'  style='width:0px;height:100%;float:right;'></div><div style='clear:both;width:0px;height: 0px;font-size: 0px;'></div>");
            ...
            SeqChartDataProperty.series = this.GetChartInitData();
            this.Set("SeqChartDataProperty", SeqChartDataProperty);
        ...
        CustomProPanelShow: function () {
            Agi.Controls.SeqChartProrityInit(this);
        }, //显示自定义属性面板
        Destory: function () {
...
//        },
        ...
        Refresh: function () {
            ...
            var BaseControlObj = new Highcharts.Chart({
                ...
                series:
                [{
		            data: [0,0,0,0.8,1.5,2,1.7,0.3,0,0,0],
		            type: 'column'
		            
		        },{
		            data: [0,0,0,0,1,2,1,0,0],
		            color: '#909596',
		            dashStyle: 'longdash',
		            type: 'spline'
		        }
		        ,{
		            data: [0,0,0,0.3,1,2.5,1,0.3,0],
		            color: '#909596',
		            type: 'spline'
		        }
        	] 
        ...
        CreateControl: function (_Config, _Target) {
            if (_Config != null) {
                if (_Target != null && _Target != "") {
                    ...
                    bolIsExt = null;
//
                     this.ReadDataBak(_Config.Entity[0]);//获取数据
...
//            ThisOutPars=null;
    ...
    }   else  if (Key == "SeqChartDataProperty") {
        var jsonData = {
        ...
        Agi.DAL.ReadData(
            {
                "MethodName": "RSeqChart", "Paras": jsonString, "CallBackFunction":function(data){
                    if(data.result=='true')
                    {
                        var datapar={
                            data:data.data
...
//SeqChart 自定义属性面板初始化显示
Agi.Controls.SeqChartProrityInit = function (_SeqChart) {
    var Me = _SeqChart;
    var ChartOptions = Me.Get("ChartOptions");
...
/*Chart 基准线相关处理-------------------------------------------------------------------*/
Namespace.register("Agi.Controls.SeqChart");
