...
Agi.Controls.SeqChart = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
        	this.shell = null;
            this.AttributeList = [];
            var ID = savedId ? savedId : "SeqChart" + Agi.Script.CreateControlGUID();
...
//            Agi.Controls.ControlDestoryByList(this);//移除控件,从列表中
        ...
        GetConfig: function () {
            ...
            SPCSingleChartControl.Control.Entity = this.Get("Entity");
            ...
            SPCSingleChartControl.Control.ColumnDefine=this.Get("columnDefine");
        ...
        dealEntityData:function(){
        	...
        	if(data && data.length>0){
        		for(var i in data){
        			...
        			//tmpobj[datetimeName]=this.dateformat(new Date(datetimeSpercificValue),'YYYY-MM-dd hh:mm:ss');
        ...
        paintChartWithFormatData:function(data){
        	...
        	if(oFormatData){
        		...
        		for(var serialName in oFormatData){
        			...
        			var serialcolor=defaultColors[(++colorIdx)%defaultColors.length];
        			//serialColors[serialName]=serialcolor;
        			Me.Get("ChartProperty")['defined_serial_color'][serialName]=serialcolor;
        			aSeqChartData.push({
    					name: serialName,
    					color:serialcolor,
    					data: onelineData,
        		...
        		//Me.Set('serialColors',serialColors);
        	}
        	return Highcharts.StockChart( {
        		credits:{
        			enabled:false
    ...
    ItemContent.load('JS/Controls/SeqChart/SeqProp.html #seqtab-1', function () {
    	...
    	if(stockdata){
    		...
    		serialColorPickerTab.find("input[id^='seqSerialColor']").each(function(){
    			...
    			var serialColors=Me.Get("ChartProperty")['defined_serial_color'];
    			...
    			thiscolorpk.spectrum({
                    ...
                    change: function (color) {
                    	var chartProperty=Me.Get("ChartProperty")['defined_serial_color'];
                    	var definecolor=color.toHexString();
                    	chartProperty[serialname]=definecolor;
