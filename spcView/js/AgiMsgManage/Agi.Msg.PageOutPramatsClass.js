...
Agi.Msg.PageOutPramatsClass = function () {
    ...
    */
    ...
    this.GetEntityParas = function () {
        var ReturnData = [];
        for (var i = 0; i < Agi.view.workspace.controlList.toArray().length; i++) {
            var temp = new FrameWork_ControlIDParaName();
            var _controlObj = Agi.view.workspace.controlList.toArray()[i];
            temp.ControlID = _controlObj.Get("ProPerty").ID; //控件ID

            //获取实体
            for (var j = 0; j < _controlObj.Get("Entity").length; j++) {
                var tempEntityName = _controlObj.Get("Entity")[j].Key;
                if (_controlObj.Get("Entity")[j].IsShareEntity != true) { //不是共享
                    var paras = _controlObj.Get("Entity")[j].Parameters;
                    if (paras != null) {
