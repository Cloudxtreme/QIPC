        ...
        RefreshCharacteristicShow:function(){
            ...
            Agi.Controls.BightChartCharacteristicTols(Me,ChartSeriesData,function(TolData){
                ...
                if(TolData.chtis.MAX.Enable && TolData.chtis.MAX.Value!=null){
                    Chartobj.addSeries({name:"最大值",data:[[TolData.pointarray[0],TolData.chtis.MAX.Value],[TolData.pointarray[1],TolData.chtis.MAX.Value]],color:"red"})
                }
                if(TolData.chtis.MIN.Enable && TolData.chtis.MIN.Value!=null){
                    Chartobj.addSeries({name:"最小值",data:[[TolData.pointarray[0],TolData.chtis.MIN.Value],[TolData.pointarray[1],TolData.chtis.MIN.Value]],color:"red"})
                }
                if(TolData.chtis.MEAN.Enable && TolData.chtis.MEAN.Value!=null){
                    Chartobj.addSeries({name:"均值",data:[[TolData.pointarray[0],TolData.chtis.MEAN.Value],[TolData.pointarray[1],TolData.chtis.MEAN.Value]],color:"red"})
                }
                if(TolData.chtis.DEVIAT.Enable && TolData.chtis.DEVIAT.Value!=null){
                    Chartobj.addSeries({name:"离差",data:[[TolData.pointarray[0],TolData.chtis.DEVIAT.Value],[TolData.pointarray[1],TolData.chtis.DEVIAT.Value]],color:"red"})
                }
                if(TolData.chtis.STED.Enable && TolData.chtis.STED.Value!=null){
                    Chartobj.addSeries({name:"标准差",data:[[TolData.pointarray[0],TolData.chtis.STED.Value],[TolData.pointarray[1],TolData.chtis.STED.Value]],color:"red"})
                }
            });
        },//特征值更新显示
...
Agi.Controls.BightChartPropertyYAxis = function (_Panel, Me) {
    ...
    ItemContentPanel.load('js/Controls/BightChart/tabTemplates.html #tab-3', function () {
        ...
        var ThisChartOption=Me.Get("ChartOption");
        if(ThisChartOption.PlotLines.yplotlinesenable){
            $("#BightChart_yplotEnable").find("option[value='No']").attr("selected","selected");
        }else{
            $("#BightChart_yplotEnable").find("option[value='OFF']").attr("selected","selected");
        }
    ...
    if(ChartOption!=null && ChartOption.PlotLines.xplotlines!=null
        ...
        for (var i = 0; i < chart.series.length; i++) {
            ...
            if(SeriesData!=null && SeriesData.length>0){
                //Agi.Controls.BightChartUpdateLoading(Me,true);
                ...
                //Agi.Controls.BightChartUpdateLoading(Me,false);
            }
        }
    }
}
...
Agi.Controls.BightChartCharacteristicTols=function(Me,SeriesData,CallbackFunction){
    ...
    if(SeriesData!=null && SeriesData.length>0){
        ...
        ChartOption.Characteristic.MAX.Value=Agi.Script.GetArrayMaxValue(ALLPointData);//获取最大值
        ChartOption.Characteristic.MIN.Value=Agi.Script.GetArrayMinValue(ALLPointData);//获取最小值
        //离差，标准差待计算
        //.......
        //回调处理
        CallbackFunction({chtis:ChartOption.Characteristic,pointarray:pointarray});
    }else{
...
//加载图标显示与隐藏控制
Agi.Controls.BightChartUpdateLoading=function(_Control,_ShowState){
    if($("#BightChartLoadingPanel").length>0){
        if(_ShowState){
            var HTMLPanelObj=$(_Control.Get("HTMLElement"));
            $("#BightChartLoadingPanel").show().css({"left":(HTMLPanelObj.width()/2-75)+"px","top":(HTMLPanelObj.height()/2-20)+"px"});
            HTMLPanelObj=null;
        }else{
            $("#BightChartLoadingPanel").hide();
        }
    }else{
        if(_ShowState){
            var BightChartLoadingPanel="<div id='BightChartLoadingPanel' class='BigChartLoadingPanel'>" +
                "<div class='BigChartLoadingTitle'>图形计算更新中，请稍后...</div>" +
                "<div class='BIgChartLoadingImgPanel'><img src='js/Controls/BightChart/Images/chartloading.gif'/></div></div>";
            var HTMLPanelObj=$(_Control.Get("HTMLElement"));
            HTMLPanelObj.append(BightChartLoadingPanel);
            $("#BightChartLoadingPanel").show().css({"left":(HTMLPanelObj.width()/2-75)+"px","top":(HTMLPanelObj.height()/2-20)+"px"});
            HTMLPanelObj=null;
        }
    }
