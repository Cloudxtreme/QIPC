...
Agi.Controls.CustomMRChart = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
...
//            cp.series_MiddlePlotLine_Enabled = false;
        ...
        ChartLoadByData:function(_Data){
            ...
            if(_Data!=null && _Data.length>0){
                ...
                Me.Set("GroupLaves",Agi.Controls.CustomMRChart.GroupSizeDataSave(DataArray,cp.chart_Data_NRow));

                var jsonData = { "action": "RCalMeanRange",
                    "name": ArrayBCName,
                    "dataArray": DataArray,
                ...
                Agi.DAL.ReadData({ "MethodName": "RCalMeanRange", "Paras": jsonString, "CallBackFunction": function (_result) {
        ...
        GetDataSeries:function(){
            ...
            var DataSeries={
                    Mean_data:Agi.Controls.CustomMRChartGetDataSries(TwoChartSeries.Mean_data,true),
                    SD_data:Agi.Controls.CustomMRChartGetDataSries(TwoChartSeries.SD_data,true)
                }
            return DataSeries;
        ...
        ShowALLPlotLineGroup:function(){
            ...
            //region 1.均值图
            var CusMRChart_Mean=Me.Get('chart');
            var PlotLineAndBands= CusMRChart_Mean.yAxis[0].plotLinesAndBands[0];
            if(PlotLineAndBands!=null&& PlotLineAndBands.length>0){
                for(var i=0;i<PlotLineAndBands.length;i++){
                    CusMRChart_Mean.yAxis[0].removePlotLine(PlotLineAndBands[i].id);
                }
            }
            ...
            if(_PlotLineGrps!=null && _PlotLineGrps.length>0){
                for(var i=0;i<_PlotLineGrps.length;i++){
                    newGroup=Agi.Controls.CustomMRChartFormatPlotGrpPars(Me.Get("Entity")[0],_PlotLineGrps[i]);//新增标准线组
                    if(newGroup!=null){
            ...
            if(PlotLines!=null){
                //获取当前标准线组的标准线元素
                var Yasixobj=CusMRChart_Mean.yAxis[0];
                for(var i=0;i<PlotLines.length;i++){
            ...
            var DataSeries=Agi.Controls.CustomMRChartGetDataSries(chartSeries.Mean_data);
            Agi.Controls.CustomMRChartPlotLineUpSeries(DataSeries,PlotLines);//更新Series 点数据
            ...
            if(WarnRuleValue!=null && WarnRuleValue.warnColumn!=null && FilterData!=null && FilterData.length>0){
                ...
                if(checkDataSeries.Mean_data!=null && checkDataSeries.Mean_data.length>0){
                    ...
                    for(var j=0;j<DataSeries[0].data.length;j++){
                        PointRows= Agi.Controls.CustomMRChart.FindDataRowsByPoint(checkDataSeries.Mean_data,DataSeries[0].data[j].x);
            ...
            if(DataSeries!=null && DataSeries.length>0){
                for(var i=0;i<CusMRChart_Mean.series.length;i++){
                    for(var j=0;j<DataSeries.length;j++){
                        if(CusMRChart_Mean.series[i].name==DataSeries[j].name){
                            CusMRChart_Mean.series[i].remove();
                            i=-1;
                            break;
                        }
                    }
                ...
                for(var i=0;i<DataSeries.length;i++){
                    DataSeries[i].data=Me.ApplySigmRule(DataSeries[i].data,checkDataSeries.Mean_data);//应用西格玛八项判异规则
                    CusMRChart_Mean.addSeries(DataSeries[i]);
                }
            }
           //endregion

            //region 2.极差图
            var CusMRChart_SD=Me.Get('chart_SD');
            var PlotLineAndBands= CusMRChart_SD.yAxis[0].plotLinesAndBands[0];
            if(PlotLineAndBands!=null&& PlotLineAndBands.length>0){
                for(var i=0;i<PlotLineAndBands.length;i++){
                    CusMRChart_SD.yAxis[0].removePlotLine(PlotLineAndBands[i].id);
                }
            }
            ...
            if(PlotLines_sd!=null){
                //获取当前标准线组的标准线元素
                var Yasixobj=CusMRChart_SD.yAxis[0];
            ...
            var DataSeries=Agi.Controls.CustomMRChartGetDataSries(chartSeries.SD_data);
            Agi.Controls.CustomMRChartPlotLineUpSeries(DataSeries,PlotLines_sd);//更新Series 点数据
            ...
            if(WarnRuleValue!=null && WarnRuleValue.warnColumn!=null && FilterData!=null && FilterData.length>0){
                ...
                if(checkDataSeries.SD_data!=null && checkDataSeries.SD_data.length>0){
                    var PointRows=null;
                    for(var j=0;j<DataSeries[0].data.length;j++){
                        PointRows= Agi.Controls.CustomMRChart.FindDataRowsByPoint(checkDataSeries.SD_data,DataSeries[0].data[j].x);
                        bool=Agi.Controls.WarnValueCompare(FilterData[PointRows[0]][WarnRuleValue.warnColumn],WarnRuleValue.warnCompareValue,WarnRuleValue.warnRule);
                        if(bool){
                            DataSeries[0].data[j].marker={fillColor:WarnRuleValue.warnColor};
                        }
            ...
            if(DataSeries!=null && DataSeries.length>0){
                for(var i=0;i<CusMRChart_SD.series.length;i++){
                    for(var j=0;j<DataSeries.length;j++){
                        if(CusMRChart_SD.series[i].name==DataSeries[j].name){
                            CusMRChart_SD.series[i].remove();
                            i=-1;
                            break;
                        }
                    }
                ...
                for(var i=0;i<DataSeries.length;i++){
                    DataSeries[i].data=Me.ApplySigmRule(DataSeries[i].data,checkDataSeries.SD_data);//应用西格玛八项判异规则
                    CusMRChart_SD.addSeries(DataSeries[i]);
                }
            }
            //endregion

//           //标准线、西格玛线 统计信息
//           var tolInfo=Agi.Controls.CustomMRChartAlarmStatistical(chartSeries,PlotLines);//统计信息
//           var AppendToPanel=Me.Get("HTMLElement");
//            var cp=Me.Get('ChartProperty');
//            tolInfo.SGM.SigmaValue=cp.ChartisigmaValue;//西格玛系数
//            Agi.Controls.CustomMRChartAlarmStatisticalTableShow(tolInfo,AppendToPanel,cp.ChartBackConfig);
        },//显示标准线
...
//            debugger;
        ...
        ApplySigmRule:function(_SeriesDataArray,_CheckDataSeries){
            var Me=this;
            ...
            if(SigmArray!=null && SigmArray.length>0){
                ...
                var NewCheckDataSries=Agi.Controls.SigmRuleApplyGetGroupData(_CheckDataSeries);//分组数据信息
                var ojb = new MRclsSigmaFunc(_SeriesDataArray,SigmArray,NewCheckDataSries);
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            this.AttributeList = [];
            this.Set("Entity", []);
            this.Set("ControlType", "CustomMRChart");
            ...
            var ID = savedId ? savedId : "CustomMRChart" + Agi.Script.CreateControlGUID();
            ...
            this.Set("Sigmrules",DefaultruleArray);
            ...
            if(checkDataSeries.Mean_data!=null && checkDataSeries.Mean_data.length>0){
                {//数据结构生成
                    ...
                    for (var i = 0; i < checkDataSeries.Mean_data.length; i++) {
                        ...
                        obj.Data = Agi.Controls.CustomMRChart.GetData(obj,cp); //获取模拟数据
        ...
        PointClick:function(_PointValue,_ChartNO){
            ...
            var GridData={
                ...
                AlarmCells:[], //报警点信息
                AbnormalRows:Agi.Controls.CustomMRChart.FindDataRowsByPoint(ThisChartCheckSeries,PointRow)//所选的异常点
            };
            ...
            if(ThisSelChartSeries!=null && ThisSelChartSeries.length>0){
                for(var i=0;i<ThisSelChartSeries.length;i++){
                    Agi.Controls.CustomMRChartAlarmDataPointsFind(GridData,cp.SeriesDataColumn[i],ThisSelChartSeries[i],cp.series_PointMarkerColor,ThisChartCheckSeries);
                }
            }
        ...
        CustomProPanelShow: function () {//显示自定义属性面板
            Agi.Controls.CustomMRChartProrityInit(this);
        },
        Destory: function () {
...
//            Agi.Controls.ControlDestoryByList(this);//移除控件,从列表中
        ...
        ControlAttributeChangeEvent: function (_obj, Key, _Value) {
            Agi.Controls.CustomMRChartAttributeChange(this, Key, _Value);
        },
        GetConfig: function () {
            var ProPerty = this.Get("ProPerty");
...
//                    alert(JSON.stringify(rowData));
        ...
        SPCViewStandLineMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomMRChartView_StandLinesMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，标准线
        SPCViewSigmaMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomMRChartView_SigmaMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，西格玛设置
        SPCViewDataExtractMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomMRChartView_ExtractMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，数据钻取
        SPCViewSpecialAlarmMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomMRChartView_SpecialAlarmMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，特殊报警
        GetSPCViewGridData:function(){
            ...
            if(ThisDataSeries!=null && ThisDataSeries.Mean_data.length>0){
                for(var i=0;i<ThisDataSeries.Mean_data.length;i++){
                    Agi.Controls.CustomMRChartAlarmDataPointsFind(GridData,cp.SeriesDataColumn[i],ThisDataSeries.Mean_data[i],cp.series_PointMarkerColor,checkDataSeries.Mean_data);
                }
            }
            if(ThisDataSeries!=null && ThisDataSeries.SD_data.length>0){
                for(var i=0;i<ThisDataSeries.SD_data.length;i++){
                    Agi.Controls.CustomMRChartAlarmDataPointsFind(GridData,cp.SeriesDataColumn[i],ThisDataSeries.SD_data[i],cp.series_PointMarkerColor,checkDataSeries.SD_data);
                }
            }
            return GridData;
        ...
        SPCViewRefreshByGridData:function(_GridData){
            ...
            //setTimeout(Agi.Controls.CustomMRChartLastChartShowExE(Me),2000);
            Agi.Controls.CustomMRChartLastChartShow(Me);
            if(Me.Get("LastChartImgIsExt")){
                Me.RefreshByProPanelUp();//更新显示
            }else{
                setTimeout(Agi.Controls.CustomMRChartRefreshShowExe(Me),2000);//更新显示
            }
            //Me.RefreshByProPanelUp();//更新显示
        ...
        SPCViewDataRestore:function(){
            ...
            Agi.Controls.CustomMRChartLastChartShowMenu(Me,false);//还原数据后，清除原对照图
...
/*下拉列表控件参数更改处理方法*/
Agi.Controls.CustomMRChartAttributeChange = function (_ControlObj, Key, _Value) {
...
//拖拽控件至编辑页面时，初始控件的方法
Agi.Controls.InitCustomMRChart = function () {
    return new Agi.Controls.CustomMRChart();
}

Agi.Controls.CustomMRChartProrityInit = function (_BasicChart) {
    var Me = _BasicChart;
    //取得默认属性
    var cp = Me.Get('ChartProperty');
    ...
    //1.将属性项加入集合,有多少个属性就需要new对应个数的 Agi.Controls.Property.PropertyItem
    {
        ...
        ItemContent.load('js/Controls/CustomMRChart/tabTemplates.html #tab-1', function () {
            $(this).find("#CustMRdataColor").spectrum({
                showInput: true,
                showPalette: true,
                palette: [
                    ['black', 'white'],
            ...
            $(this).find("#CustMRmarkerColor").spectrum({
                showInput: true,
                showPalette: true,
                palette: [
                    ['black', 'white'],
            ...
            //默认选中
            $(this).find("#CustMRdataColor").spectrum("set","#d2d2d2");
            $(this).find("#CustMRmarkerColor").spectrum("set","#4572A7");
            if(cp.series_DataColor!=null && cp.series_PointMarkerColor!=null){
                $(this).find("#CustMRdataColor").spectrum("set",cp.series_DataColor);
                $(this).find('#CustMRmarkerColor').spectrum("set",cp.series_PointMarkerColor);
            }
            //分组列和数据列绑定
            var GroupColumns=$(this).find('#CustMRGrpColumn');
            var DataColumns=$(this).find('#CustMRDataColumn');
            GroupColumns.empty();
            DataColumns.empty();
            ...
            if(cp.SeriesGroupColumn && cp.SeriesDataColumn!=null){
                ...
                if(cp.SeriesGroupColumn==null){
                    cp.SeriesGroupColumn=$($("#CustMRGrpColumn").children()[0]).val();
                }
                if(cp.SeriesDataColumn==null){
                    cp.SeriesDataColumn=$($("#CustMRDataColumn").children()[0]).val();
                }
            }
            ...
            var CustMRChartSeriesItems= $(this).find("#CustMRSeries").empty();
            if(ChartDataSeries!=null && ChartDataSeries.length>0){
                for(var i=0;i<ChartDataSeries.length;i++) {
                    ...
                    CustMRChartSeriesItems.append(option);
                }
                CustMRChartSeriesItems.find("option[value='"+ChartDataSeries[0].name+"']").attr("selected","selected");
            }
            //子组大小初始化
            if(cp.chart_Data_NRow==null){cp.chart_Data_NRow=2};
            $("#MRChart_RGroupSize").val(cp.chart_Data_NRow);

            //保存处理
            $(this).find("#CustMRSeriesSave").unbind().bind("click",function(ev){
                var CustMRChartSeriesObj={
                    SeriesName:$("#CustMRSeries").val(),
                    GroupColumn:$("#CustMRGrpColumn").val(),
                    DataColumn:$("#CustMRDataColumn").val(),
                    SeriesColor:$("#CustMRdataColor").val(),
                    SeriesMarkerColor:$("#CustMRmarkerColor").val(),
                    GroupSize:$("#MRChart_RGroupSize").val()//分组大小
                }
                Me.UpSeriesBindInfo(CustMRChartSeriesObj);//更新Series 设置
        ...
        Agi.Controls.CustomMRChartView_BackgroundSetMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMRChartView_SigmaSettingMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMRChartView_RTypeMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMRChartView_SigmaMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMRChartView_StandLinesMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomMRChartView_StandLinesMenu_SD(ItemContent,Me);
        ThisProItems.push(new Agi.Controls.Property.PropertyItem({ Title: "样本极差_标准线", DisabledValue: 1, ContentObj: ItemContent }));
        //endregion
...
//        ItemContent = null;
//        ItemContent=$("<div id='CSChartSigMa'></div>");
//        Agi.Controls.CustomMRChartView_SpecialAlarmMenu(ItemContent,Me);
...
//        Agi.Controls.CustomMRChartView_ExtractMenu(ItemContent,Me);
...
//1.格式化标准线组的显示值信息
Agi.Controls.CustomMRChartFormatPlotGrpPars=function(_Entity,_PlotGrp){
    var PlotArray=null;
...
Agi.Controls.CustomMRChartPlotLineUpSeries=function(_SeriesArrar,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
//    var DataSeriesArray=Agi.Controls.CustomMRChartGetDataSries(_SeriesArrar);
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
            Agi.Controls.CustomMRChartPlotLineUpPoint(_SeriesArrar[i],_PlotLines);
        }
    }
}
Agi.Controls.CustomMRChartGetDataSries=function(_SeriesArrar,_IsDataSeries){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    var SeriesArray=[];
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
...
Agi.Controls.CustomMRChartPlotLineUpPoint=function(_Series,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    if(_Series!=null && _Series.data!=null && _Series.data.length>0){
        var PointColor=null;
        for(var i=0;i<_Series.data.length;i++){
            PointColor=Agi.Controls.CustomMRChartPlotLineGetPointColor(_Series.data[i].y,_PlotLines);
            if(PointColor!=null){
...
Agi.Controls.CustomMRChartPlotLineGetPointColor=function(_Value,_PlotLines){
    var PointColor=null;
...
//统计信息
Agi.Controls.CustomMRChartAlarmStatistical=function(_ALLSeries,_PlotLines){
    ...
    var _Series=Agi.Controls.CustomMRChartGetDataSries(_ALLSeries);//获得对应的的数据Series
    var _ScmSeries=Agi.Controls.CustomMRChartGetDataSries(_ALLSeries,false);//获得对应的的西格玛线Series
    ...
    if(_Series!=null || _ScmSeries!=null){
        ...
        for(var i=0;i<_Series.length;i++){
            for(var j=0;j<_Series[i].data.length;j++){
                Agi.Controls.CustomMRChartAlarmStatisticalByData(j,_Series[i].data[j].y,AlarmStatisticaldata,SumPointNum);//统计标准线所在比例信息
            }
        }
    }
...
Agi.Controls.CustomMRChartAlarmStatisticalByData=function(_index,_PointValue,_AlarmStatisticaldata,SumPointNum){
    //标准线超出点计算
...
//统计信息显示
Agi.Controls.CustomMRChartAlarmStatisticalPanelShow=function(_AlarmStatisticaldata,_ApendToPanel){
    if($("#CustomMRChartAlarmPanel").length>0){
        $("#CustomMRChartAlarmPanel").html("");
    }else{
        var CustomMRChartAlarmPanelHTML=$("<div id='CustomMRChartAlarmPanel' class='AlarmSty'></div>").appendTo($(_ApendToPanel));
        $("#CustomMRChartAlarmPanel").css({"right":"8px","top":"8px"});
    }
    var SubItemHTML="";
    ...
    $("#CustomMRChartAlarmPanel").append(SubItemHTML);
}
...
Agi.Controls.SigmRuleApplyGetGroupData=function(_CheckedDataArray){
    var GroupData=[];
...
//2.查找异常点
Agi.Controls.CustomMRChartAlarmDataPointsFind=function(_GridData,_ColumnName,_ChartSeires,_markercolor,_CheckDataSeries){
    var _Index=-1;
    ...
    if(_Index>-1){
        ...
        for(var j=0;j<_ChartSeires.data.length;j++){
            if(_ChartSeires.data[j].marker.fillColor!=_markercolor){//如果marker点颜色与原设置point点颜色不一致则说明该点为报警点
                NewItemArray=Agi.Controls.CustomMRChart.FindDataRowsByPoint(_CheckDataSeries,_ChartSeires.data[j].x);
...
Agi.Controls.CustomMRChartView_StandLinesMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('js/Controls/CustomMRChart/tabTemplates.html #tab-2', function () {
        //0.控件当前值获取
        ...
        $(this).find("#btnNewGrp").click(function(){
            ...
            LineTablePanel.find('#CustMRStLinename').show();
            LineTablePanel.find('#CustMRStLinegroupName').hide();
        ...
        $(this).find("#btnSaveGrp").click(function(){
            ...
            if($("#CustMRStLinegroupName").is(":hidden")){
                if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
                    ...
                    if(Intopstatevalue==0){//新增
                        for(var i=0;i<ThisChartStandLines.length;i++){
                            if($("#CustMRStLinename").attr('value')==ThisChartStandLines[i].GroupName)
                            {
                        ...
                        var NewGrpName=$("#CustMRStLinename").attr('value');
                        var OldSelGrpName=$("#CustMRStLinegroupName").val();
                        var indexA=-1;
                        var indexB=-1;
                ...
                var SelValue=$("#CustMRStLinegroupName").val();
                if(SelValue!=null && SelValue!=""){
                    SelectedItemObj=SelValue;
                }
            }
            $('#CustMRStLinename').hide();
            $('#CustMRStLinegroupName').show();
            $('#btnGrpUndo').hide();
            $('#btnEditGrp').show();
            $("#btnNewGrp").show();
            $("#btnDeleteGrp").show();
            ...
            var line={
                GroupName:$("#CustMRStLinename").attr('value'),
            ...
            var StandLinGrpNames = $("#CustMRStLinegroupName");
        ...
        $(this).find('#btnDeleteGrp').click(function(){
            if($('#CustMRStLinegroupName').css('display')!="none"){
                var index=-1;
                for(var i=0;i<ThisChartStandLines.length;i++){
                    if($('#CustMRStLinegroupName option:selected').text()==ThisChartStandLines[i].GroupName)
                    {
                        index=i;break;
                    }
                }
                ThisChartStandLines.splice(index,1);
                var name = $("#CustMRStLinegroupName");
                name.empty();
                for(var i=0;i<ThisChartStandLines.length;i++) {
                    var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>"
                    name.append(option);
                ...
                {
                    $('#CustMRStLinegroupName').hide();
                    $('#CustMRStLinename').val("").show();
                }
                groupNameChange(ThisChartStandLines);
            }
            else{
        ...
        $(this).find("#btnEditGrp").unbind().bind("click",function(ev){
            var LineTablePanel=$(this).parent().parent().parent();
            LineTablePanel.find('#CustMRStLinename').show().val(LineTablePanel.find('#CustMRStLinegroupName').val());
            LineTablePanel.find('#CustMRStLinegroupName').hide();
        ...
        $(this).find("#CustMRStLinegroupName").change(function(){
            debugger;
            groupNameChange(ThisChartStandLines);
        });
        ...
        var groupNameChange=function(_StandLines){
            ...
            {
                if($("#CustMRStLinegroupName  option:selected").text()==_StandLines[i].GroupName){
        ...
        //4.初始化绑定
        var StandLinGrpNames = $(this).find("#CustMRStLinegroupName");
        StandLinGrpNames.empty();
        if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
            for(var i=0;i<ThisChartStandLines.length;i++) {
                var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>";
            ...
            StandLinGrpNames.find("option[value='"+ThisChartStandLines[0].GroupName+"']").attr("selected",true);
            $(this).find('#CustMRStLinename').hide();
            $(this).find('#CustMRStLinegroupName').show();
            groupNameChange(ThisChartStandLines);//绑定显示

            $("#btnEditGrp").show();
        }else{
            $(this).find('#CustMRStLinename').show();
            $(this).find('#CustMRStLinegroupName').hide();
        }
...
//3.2.样本极差标准线
Agi.Controls.CustomMRChartView_StandLinesMenu_SD=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('js/Controls/CustomMRChart/tabTemplates.html #tab-10', function () {
        //0.控件当前值获取
        ...
        $(this).find("#btnNewGrp_SD").click(function(){
            //清空面板
            var LineTablePanel=$(this).parent().parent().parent();
            LineTablePanel.find('#CustMRStLinename_SD').show();
            LineTablePanel.find('#CustMRStLinegroupName_SD').hide();
        ...
        $(this).find("#btnSaveGrp_SD").click(function(){
            ...
            if($("#CustMRStLinegroupName_SD").is(":hidden")){
                if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
                    //创建对象
                    if(Intopstatevalue==0){//新增
                        for(var i=0;i<ThisChartStandLines.length;i++){
                            if($("#CustMRStLinename_SD").attr('value')==ThisChartStandLines[i].GroupName)
                            {
                                AgiCommonDialogBox.Alert('标准线不允许重名！');
                                return;
                            }
                        ...
                        var NewGrpName=$("#CustMRStLinename_SD").attr('value');
                        var OldSelGrpName=$("#CustMRStLinegroupName_SD").val();
                        var indexA=-1;
                        var indexB=-1;
                        if(NewGrpName!=OldSelGrpName){
                            for(var i=0;i<ThisChartStandLines.length;i++){
                ...
                var SelValue=$("#CustMRStLinegroupName_SD").val();
                if(SelValue!=null && SelValue!=""){
                    SelectedItemObj=SelValue;
                }
            }
            $('#CustMRStLinename_SD').hide();
            $('#CustMRStLinegroupName_SD').show();
            $('#btnGrpUndo_SD').hide();
            $('#btnEditGrp_SD').show();
            $("#btnNewGrp_SD").show();
            ...
            var line={
                GroupName:$("#CustMRStLinename_SD").attr('value'),
            ...
            var StandLinGrpNames = $("#CustMRStLinegroupName_SD");
            StandLinGrpNames.empty();
            if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
                for(var i=0;i<ThisChartStandLines.length;i++) {
                    var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>";
        ...
        $(this).find('#btnDeleteGrp_SD').click(function(){
            if($('#CustMRStLinegroupName_SD').css('display')!="none"){
                var index=-1;
                for(var i=0;i<ThisChartStandLines.length;i++){
                    if($('#CustMRStLinegroupName_SD option:selected').text()==ThisChartStandLines[i].GroupName)
                    {
                        index=i;break;
                    }
                }
                ThisChartStandLines.splice(index,1);
                var name = $("#CustMRStLinegroupName_SD");
                name.empty();
                for(var i=0;i<ThisChartStandLines.length;i++) {
                    var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>"
                    name.append(option);
                ...
                if(ThisChartStandLines.length==0)
                {
                    $('#CustMRStLinegroupName_SD').hide();
                    $('#CustMRStLinename_SD').val("").show();
                }
                groupNameChange(ThisChartStandLines);
            }
            else{
        ...
        $(this).find("#btnEditGrp_SD").unbind().bind("click",function(ev){
            var LineTablePanel=$(this).parent().parent().parent();
            LineTablePanel.find('#CustMRStLinename_SD').show().val(LineTablePanel.find('#CustMRStLinegroupName_SD').val());
            LineTablePanel.find('#CustMRStLinegroupName_SD').hide();
        ...
        $(this).find("#CustMRStLinegroupName_SD").change(function(){
            debugger;
            groupNameChange(ThisChartStandLines);
        });
        ...
        var groupNameChange=function(_StandLines){
            for(var i=0;i<_StandLines.length;i++)
            {
                if($("#CustMRStLinegroupName_SD  option:selected").text()==_StandLines[i].GroupName){
        ...
        //4.初始化绑定
        var StandLinGrpNames = $(this).find("#CustMRStLinegroupName_SD");
        StandLinGrpNames.empty();
        if(ThisChartStandLines!=null && ThisChartStandLines.length>0){
            for(var i=0;i<ThisChartStandLines.length;i++) {
                var option ="<option value='"+ThisChartStandLines[i].GroupName+"'>"+ThisChartStandLines[i].GroupName+"</option>";
            ...
            StandLinGrpNames.find("option[value='"+ThisChartStandLines[0].GroupName+"']").attr("selected",true);
            $(this).find('#CustMRStLinename_SD').hide();
            $(this).find('#CustMRStLinegroupName_SD').show();
            groupNameChange(ThisChartStandLines);//绑定显示

            $("#btnEditGrp_SD").show();
        }else{
            $(this).find('#CustMRStLinename_SD').show();
            $(this).find('#CustMRStLinegroupName_SD').hide();
        }
...
Agi.Controls.CustomMRChartView_SigmaSettingMenu=function(_Panel,_Control){
    var Me=_Control;
    var ItemContentPanel=$(_Panel);
    if($(_Panel).find(".content").length>0){
        ItemContentPanel=$(_Panel).find(".content");
    }
    ItemContentPanel.load('js/Controls/CustomMRChart/tabTemplates.html #tab-6', function () {
        ...
        $(this).find("#CustMRSigmaValue").val(cp.ChartisigmaValue);
        var VisibleStateValue="1";
        if(!cp.ChartisigmaLineVisible){
            VisibleStateValue="0";
        }
        $(this).find("#CustMRSigmaLineState").find("option[value='"+VisibleStateValue+"']").attr("selected",true);
        cp=null;
    });
}
//4.在View环境中显示西格玛判异菜单项
Agi.Controls.CustomMRChartView_SigmaMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('js/Controls/CustomMRChart/tabTemplates.html #tab-3', function () {
        var THisChartSigmaRules=Me.Get("Sigmrules");//获取八项判异规则
        //1.八项规则保存
        $(this).find("#CustMRSigMaRuleSave").unbind().bind("click",function(ev){
            var rule=[];
            var ruleitem=null;
            for(i=1;i<=8;i++)
            {
...
//5.特殊报警
Agi.Controls.CustomMRChartView_SpecialAlarmMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('js/Controls/CustomMRChart/tabTemplates.html #tab-5', function () {
        //1.保存规则
...
//6.数据钻取设置
Agi.Controls.CustomMRChartView_ExtractMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('js/Controls/CustomMRChart/tabTemplates.html #tab-7', function () {
        //0.默认选项隐藏
        $("#CustomMRParCancel").hide();
        $("#CustomMRNewParsTxt").width(0).hide();

        //1.添加参数
        $(this).find('#CustomMRParAdd').click(function(){
            $(this).hide();
            $("#CustomMRParCancel").show();
            $("#CustomMRNewParsTxt").width(60).show();
            $("#CustomMRParsNames").hide();
        });
        //2.取消添加
        $(this).find('#CustomMRParCancel').click(function(){
            $("#CustomMRParCancel").hide();
            $("#CustomMRNewParsTxt").width(0).val("").hide();
            $("#CustomMRParsNames").show();
            $("#CustomMRParAdd").show();
        });
        //3.保存
        $(this).find('#CustomMRParSave').click(function(){
            var SCSTxtVisibly=!$("#CustomMRNewParsTxt").is(':hidden');
            var CusChartParsName="";
            var CusChartParsValue="";
            if(SCSTxtVisibly){
                //新增参数
                CusChartParsName=$("#CustomMRNewParsTxt").val();
                CusChartParsValue=$("#CustomMRParsValues").val();
            }else{
                //编辑参数
                CusChartParsName=$("#CustomMRParsNames").val();
                CusChartParsValue=$("#CustomMRParsValues").val();
            }
                ...
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    ...
                    if(SCSTxtVisibly){
                        if(ParIndex>-1){
                            ...
                            $("#CustomMRParsNames").append("<option value='"+CusChartParsName+"'>"+CusChartParsName+"</option>");
                            ...
                            $("#CustomMRParsNames").val(CusChartParsName);
                            //更新列表显示
                            Agi.Controls.CustomMRChartParsListUpdate(ChartExtractConfig.Paras,"ParsListPanel");
        ...
        //4.删除
        $(this).find('#CustomMRParDel').click(function(){
            var SCSTxtVisibly=!$("#CustomMRNewParsTxt").is(':hidden');
            if(SCSTxtVisibly){
                AgiCommonDialogBox.Alert("请取消或完成新增，选择已存在的参数名称后再试!");
            }else{
                //编辑参数
               var CusChartParsName=$("#CustomMRParsNames").val();
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    var ParIndex=-1;
                    for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                        if(ChartExtractConfig.Paras[i].Name==CusChartParsName){
                    ...
                    if(ParIndex>-1){
                        //移除对应的Option项
                        $("#CustomMRParsNames option[value='"+ChartExtractConfig.Paras[ParIndex].Name+"']").remove(); //删除Select中Value=''的Option
        ...
        Agi.PageDataDrill.GetAllPageFilList(function(_result){
            if(_result.result && _result.result=="true"){
                ...
                if(_result.data!=null && _result.data.length>0){
                    ...
                    itemValue=null;
                    $("#CustMRExtractURL").html(strCustSigChartExtractURLoptions);
                    if(ChartExtractConfig!=null && ChartExtractConfig.URL!=null){
                        $("#CustMRExtractURL").find("[value='"+ChartExtractConfig.URL+"']").attr("selected","selected");
                    }
                }
            }
        })
        if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
            for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                $("#CustomMRParsNames").append("<option value='"+ChartExtractConfig.Paras[i].Name+"'>"+ChartExtractConfig.Paras[i].Name+"</option>");
            }
            DefaultParNameColumn=ChartExtractConfig.Paras[0].Column;
            $("#CustomMRParsNames").val(ChartExtractConfig.Paras[0].Name);
        ...
        if(ChartEntityColumns!=null && ChartEntityColumns.length>0){
            for(var i=0;i<ChartEntityColumns.length;i++){
                $("#CustomMRParsValues").append("<option value='"+ChartEntityColumns[i]+"'>"+ChartEntityColumns[i]+"</option>");
            }
            if(DefaultParNameColumn!=null){
                $("#CustomMRParsValues").val(DefaultParNameColumn);
            }
        }
        //6.参数名称选中项更改
        $(this).find('#CustomMRParsNames').change(function(){
            ...
            if(ThisParas!=null && ThisParas.length>0){
                for(var i=0;i<ThisParas.length;i++){
                    if(ThisParas[i].Name==StrThisSelValue){
                        $("#CustomMRParsValues").val(ThisParas[i].Column);
                        break;
                    }
                }
            }
        ...
        //7.参数项&URL信息保存
        $(this).find('#CstMRExtractOptionSave').click(function(){
            var ThisExtractConfig=Me.Get("ExtractConfig");//数据钻取配置信息保存
            ThisExtractConfig.URL=$("#CustMRExtractURL").val();
...
//7.背景设置
Agi.Controls.CustomMRChartView_BackgroundSetMenu=function(_Panel,_Control){
    var Me=_Control;
    //获取属性
    var cp = Me.Get('ChartProperty');
    ...
    ItemContentPanel.load('js/Controls/CustomMRChart/tabTemplates.html #tab-8', function () {
        $(this).find(".CustomMRColorSty").spectrum({
            showInput: true,
            showPalette: true,
            palette: [
                ['black', 'white'],
        ...
        $("#CustomMR_bgvalue").unbind().bind("click",function(){
        ...
        var ThisChartbgvalue=Agi.Controls.CustomMRChartBackgroundValueGet(cp.ChartBackConfig.backgroundColor);
        $("#CustomMR_bgvalue").css("background",ThisChartbgvalue.value.background).data('colorValue',ThisChartbgvalue);//设置默认项
        // backgroundColor//背景 ,Tolbtnbackground,//菜单按钮背景,TolbtnFontColor,//菜单按钮字体,TolTabbackground,//统计表格背景,TolTabFontColor//统计表格字体
        $("#CustomMR_Tolbgvalue").spectrum("set",cp.ChartBackConfig.Tolbtnbackground);
        $("#CustomMR_Tolbtncolor").spectrum("set",cp.ChartBackConfig.TolbtnFontColor);
        $("#CustomMR_TolTabbg").spectrum("set",cp.ChartBackConfig.TolTabbackground);
        $("#CustomMR_TolTabfontbg").spectrum("set",cp.ChartBackConfig.TolTabFontColor);

        $("#CustomMR_BackgroundSave").unbind().bind("click",function(){
            var color=$("#CustomMR_bgvalue").data('colorValue');
            ...
            var BackgroudObj={
                ...
                Tolbtnbackground:$("#CustomMR_Tolbgvalue").val(),//统计信息按钮背景
                TolbtnFontColor:$("#CustomMR_Tolbtncolor").val(),//统计信息按钮字体颜色
                TolTabbackground:$("#CustomMR_TolTabbg").val(),//统计信息表格背景
                TolTabFontColor:$("#CustomMR_TolTabfontbg").val()//统计信息表格字体颜色
            }
            Agi.Controls.CustomMRChart.BackgroundApply(Me,BackgroudObj);//背景应用
            Me.RefreshByProPanelUp();//更新属性后，刷新图形显示
        })
    });
}
//9.标准差估计
Agi.Controls.CustomMRChartView_RTypeMenu=function(_Panel,_Control){
    var Me=_Control;
    var cp = Me.Get('ChartProperty');
    //获取属性
    var ItemContentPanel=$(_Panel);
    ...
    ItemContentPanel.load('js/Controls/CustomMRChart/tabTemplates.html #tab-9', function () {
        $("#CustomMR_RTypeSave").unbind().bind("click",function(){
            cp= Me.Get('ChartProperty');
            cp.optionVal=$('input:radio[name="CustomMR_RTypeRadio"]:checked').val();
            Me.Set('ChartProperty',cp);
            Me.RefreshByProPanelUp();//更新属性后，刷新图形显示
        });
...
Agi.Controls.CustomMRChartAlarmStatisticalTableShow=function(_AlarmStatisticaldata,_ApendToPanel,_BackGroundConfig){
    ...
    if($("#CustomMRChartAlarmPanel").length>0){
        $("#CustomMRChartAlarmPanel").html("<a id='btnTotalMenu' class='totalAlarmmenusty' >+统计信息</a></div>");
    }else{
        var CustomMRChartAlarmPanelHTML=$("<div id='CustomMRChartAlarmPanel' class='AlarmSty'></div>").appendTo($(_ApendToPanel));
        $("#CustomMRChartAlarmPanel").css({"right":"8px","top":"8px"});
        CustomMRChartAlarmPanelHTML.append("<a  id='btnTotalMenu' class='totalAlarmmenusty'>+统计信息</a>");
    }
    ...
    SubItemHTML+="</table>"
    $("#CustomMRChartAlarmPanel").append(SubItemHTML);
    $("#CustomMRChartAlarmPanel").find(".totalAlarmmenusty").css({
        "color":_BackGroundConfig.TolbtnFontColor,
        "background-color":_BackGroundConfig.Tolbtnbackground
    })
    $("#CustomMRChartAlarmPanel").find("#TotalTable").css({
    ...
    $("#CustomMRChartAlarmPanel").unbind().bind("dblclick",function(ev){
        ev.stopPropagation();    //  阻止事件冒泡
    });
...
//endregion
//region 编辑数据后显示对照图形
Agi.Controls.CustomMRChartLastChartShow=function(_Controls){
    var Me=_Controls;
    if(Me.Get("LastChartImgIsExt")){
        Agi.Controls.CustomMRChartLastChartShowMenu(Me,true);
    }else{
        var _canvas = [];
        ...
        html2canvas($("#"+ControlPanelID)[0],{
            onrendered:function(canvas, a){
                ...
                var ThisControlObj= Me.Get('chart');
               // $(ThisControlObj.renderTo).append("<div id='"+ControlPanelID+"_Img' class='MRChartLastImgPanel'><img src='"+canvas.toDataURL('image/png')+"' style='width:100%;height:100%;'></div>");
                $("#"+ControlPanelID).append("<div id='"+ControlPanelID+"_Img' class='MRChartLastImgPanel'><img src='"+canvas.toDataURL('image/png')+"' style='width:100%;height:100%;'></div>");
...
Agi.Controls.CustomMRChartRefreshShowExe=function(_Controls){
    return function(){
        _Controls.RefreshByProPanelUp();
    };
}

Agi.Controls.CustomMRChartLastChartShowMenu=function(_Control,_IsShow){
    var Me=_Control;
    var _ApendToPanel=Me.Get("HTMLElement");
    if(_IsShow){
        if($("#CustomMRChartLastImgMenu").length>0){
        }else{
            var CustomMRChartLastImgHTML=$("<div id='CustomMRChartLastImgMenu' class='AlarmStyLasgImg' data-state='0'>查看原始图形</div>").appendTo($(_ApendToPanel));
            $("#CustomMRChartLastImgMenu").css({"right":"78px","top":"8px"});
        }
        $("#CustomMRChartLastImgMenu").show();
        $("#CustomMRChartLastImgMenu").unbind().bind("click",function(ev){
            var ControlPanelID=$(Me.Get('HTMLElement'))[0].id;
        ...
        $("#CustomMRChartLastImgMenu").hide();//隐藏
    }

}
//显示均值极差图数据钻取参数列表
Agi.Controls.CustomMRChartParsListUpdate=function(_Paras,_ListPanelID){
    var StrListItems="";
    if(_Paras!=null && _Paras.length>0){
        for(var i=0;i<_Paras.length;i++){
            StrListItems+=" <div class='ParsListHeadSty'><div class='CustomMRParsListCellSty'>"+_Paras[i].Name+"</div><div class='CustomMRParsListCellSty'>"+_Paras[i].Column+"</div></div>";
        }
    }
...
//endregion
//region 背景设置
Agi.Controls.CustomMRChartBackgroundValueGet=function(_oBackgroundObj){
...
Agi.Controls.CustomMRChart.BackgroundApply=function(_ControlObj,_BackGroudObj){
...
Agi.Controls.CustomMRChart.FindDataRowsByPoint=function(_checkDataSeries,_PointX){
//    var checkDataSeries = this.Get('CheckDataSeries');
    var PointRows=[];
...
Agi.Controls.CustomMRChart.GetData=function(objValue,_Chartcp) {
    var minValue = objValue.minValue;
    var maxValue = objValue.maxValue;
    var objData = [];
...
//计算分组后的剩余数据行数
Agi.Controls.CustomMRChart.GroupSizeDataSave=function(_GroupData,_nrow){
