...
Agi.Controls.CustomUChart = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
        ...
        ChartLoadByData:function(_Data){
            ...
            var jsonData = { "action": "RCalp_Ulot",
            ...
            if(_Data!=null && _Data.length>0){
                ...
                Agi.DAL.ReadData({"MethodName":"RCalp_Ulot","Paras": jsonString, "CallBackFunction": function (_result) {
                    if (_result && _result.result == "true" && _result.groupdata!=null && _result.groupdata.length>0) {
                        ...
                        for (var i = 0; i < _result.groupdata.length; i++) {
                            ...
                            checkDataSeries.push({
                                ...
                                min: min,
                                max: max,
                                minValue: _result.groupdata[i].U_LCL,
                                middleValue: _result.groupdata[i].Ubar,
                                maxValue: _result.groupdata[i].U_UCL,
                                Isigma:null,
        ...
        GetDataSeries:function(){
            var Me=this;
            var DataSeries=Agi.Controls.CustomUChartGetDataSries(Me.Get('ChartSeries'),true);
            return DataSeries;
        ...
        ShowALLPlotLineGroup:function(){
            ...
            if(_PlotLineGrps!=null && _PlotLineGrps.length>0){
                for(var i=0;i<_PlotLineGrps.length;i++){
                    newGroup=Agi.Controls.CustomUChartFormatPlotGrpPars(Me.Get("Entity")[0],_PlotLineGrps[i]);//新增标准线组
                    if(newGroup!=null){
            ...
            var DataSeries=Agi.Controls.CustomUChartGetDataSries(chartSeries);
            Agi.Controls.CustomUChartPlotLineUpSeries(DataSeries,PlotLines);//更新Series 点数据
            ...
            if(WarnRuleValue!=null && WarnRuleValue.warnColumn!=null && FilterData!=null && FilterData.length>0){
                ...
                if(checkDataSeries!=null && checkDataSeries.length>0){
                    ...
                    for(var j=0;j<DataSeries[0].data.length;j++){
                        PointRows= Agi.Controls.CustomUChart.FindDataRowsByPoint(checkDataSeries,DataSeries[0].data[j].x);
...
//            debugger;
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            this.AttributeList = [];
            this.Set("Entity", []);
            this.Set("ControlType", "CustomUChart");
            ...
            var ID = savedId ? savedId : "CustomUChart" + Agi.Script.CreateControlGUID();
        ...
        SetChartProperty: function (_objCanvasAreaID) {//初始化定义图形控件属性结构，并预设值属性，_objCanvasAreaID：画布显示区域ID
            ...
            var chartProperty = {
                ...
                yAxis_Title_Text: 'U 图', //'单独值', //Y轴标题文字
                ...
                series_MinPlotLine_Names: "U_LCL", //是否显示最低参考线 markeluo 20121126 LCL与UCL 写反了
                series_MiddlePlotLine_Color: "green", //中间值参考线颜色
                series_MiddlePlotLine_Enabled: true, //是否显示中间参考线
                series_MiddlePlotLine_Names: "UBar", //是否显示最低参考线
                series_MaxPlotLine_Color: "#f41d12", //最大参考线颜色
                series_MaxPlotLine_Enabled: true, //是否显示最大参考线
                series_MaxPlotLine_Names: "U_UCL", //是否显示最低参考线 markeluo 20121126 LCL与UCL 写反了
        ...
        SetChartDataProperty: function () {//数据基本结构定义，在警戒范围中生成模拟数据
            ...
            {//数据结构生成
                ...
                for (var i = 0; i < checkDataSeries.length; i++) {
                    ...
                    Agi.Controls.CustomUChart.MinMaxLineData(obj,dataMin,true,"min",i);
...
//                    dataMiddle.push([obj.max, obj.middleValue]);
                    Agi.Controls.CustomUChart.MinMaxLineData(obj,dataMiddle,false,"midd",i)
                    if (i != checkDataSeries.length - 1) dataMiddle.push(null);

                    //判断并填充上下标准线的Series 点
                    Agi.Controls.CustomUChart.MinMaxLineData(obj,dataMax,false,"max",i);
        ...
        PointClick:function(_PointValue){
            ...
            var GridData={
                ...
                AlarmCells:[], //报警点信息
                AbnormalRows:Agi.Controls.CustomUChart.FindDataRowsByPoint(checkDataSeries,PointRow)//所选的异常点
            };
...
//                {GroupName:"标准线2",MaxSize:1,MaxColor:'#04fe45',MaxValueType:0,MaxValue:29000,MinSize:1,MinColor:'#fbf806',MinValueType:0,MinValue:25800, MaxReviseSig:上限修正符号,MaxReviseValue:上限修正值,MinReviseSig:下限修正符号,MinReviseValue:下限修正值}]);//添加标准线
        ...
        CustomProPanelShow: function () {//显示自定义属性面板
            Agi.Controls.CustomUChartProrityInit(this);
        },
        Destory: function () {
...
//            Agi.Controls.ControlDestoryByList(this);//移除控件,从列表中
        ...
        ControlAttributeChangeEvent: function (_obj, Key, _Value) {
            Agi.Controls.CustomUChartAttributeChange(this, Key, _Value);
        },
        GetConfig: function () {
            var ProPerty = this.Get("ProPerty");
...
//                    alert(JSON.stringify(rowData));
        ...
        SPCViewStandLineMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomUChartView_StandLinesMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，标准线
        SPCViewSigmaMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomUChartView_SigmaMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，西格玛设置
        SPCViewDataExtractMenu: function (_Panel, _CallFun) {
            ...
            Agi.Controls.CustomUChartView_ExtractMenu(_Panel, Me);
            _CallFun();
        },//SPC控件View，数据钻取
        SPCViewChartSetting:function(_Panel,_CallFun){
            ...
            $(_Panel).find(".content").height(200);
            Agi.Controls.CustomUChartView_ChartSetMenu(_Panel, Me);
            _CallFun();
        },//图形选项
...
/*下拉列表控件参数更改处理方法*/
Agi.Controls.CustomUChartAttributeChange = function (_ControlObj, Key, _Value) {
...
//拖拽控件至编辑页面时，初始控件的方法
Agi.Controls.InitCustomUChart = function () {
    return new Agi.Controls.CustomUChart();
}

Agi.Controls.CustomUChartProrityInit = function (_BasicChart) {
    var Me = _BasicChart;
    //取得默认属性
    var cp = Me.Get('ChartProperty');
    ...
    //1.将属性项加入集合,有多少个属性就需要new对应个数的 Agi.Controls.Property.PropertyItem
    {
        ...
        Agi.Controls.CustomUChartView_ChartSetMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomUChartView_BackgroundSetMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomUChartView_StandLinesMenu(ItemContent,Me);
        ...
        Agi.Controls.CustomUChartView_SigmaMenu(ItemContent,Me);
...
//        Agi.Controls.CustomUChartView_ExtractMenu(ItemContent,Me);
...
//1.格式化标准线组的显示值信息
Agi.Controls.CustomUChartFormatPlotGrpPars=function(_Entity,_PlotGrp){
    var PlotArray=null;
...
Agi.Controls.CustomUChartPlotLineUpSeries=function(_SeriesArrar,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
//    var DataSeriesArray=Agi.Controls.CustomUChartGetDataSries(_SeriesArrar);
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
            Agi.Controls.CustomUChartPlotLineUpPoint(_SeriesArrar[i],_PlotLines);
        }
    }
}
Agi.Controls.CustomUChartGetDataSries=function(_SeriesArrar,_IsDataSeries){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    var SeriesArray=[];
    if(_SeriesArrar!=null && _SeriesArrar.length>0){
        for(var i=0;i<_SeriesArrar.length;i++){
...
Agi.Controls.CustomUChartPlotLineUpPoint=function(_Series,_PlotLines){
    //_Series中元素名称 如果不为max/middle/min 则为对应的数据曲线的series
    if(_Series!=null && _Series.data!=null && _Series.data.length>0){
        var PointColor=null;
        for(var i=0;i<_Series.data.length;i++){
            PointColor=Agi.Controls.CustomUChartPlotLineGetPointColor(_Series.data[i].y,_PlotLines);
            if(PointColor!=null){
...
Agi.Controls.CustomUChartPlotLineGetPointColor=function(_Value,_PlotLines){
    var PointColor=null;
...
//统计信息
Agi.Controls.CustomUChartAlarmStatistical=function(_ALLSeries,_PlotLines){
    ...
    var _Series=Agi.Controls.CustomUChartGetDataSries(_ALLSeries);//获得对应的的数据Series
    var _ScmSeries=Agi.Controls.CustomUChartGetDataSries(_ALLSeries,false);//获得对应的的西格玛线Series
    ...
    if(_Series!=null || _ScmSeries!=null){
        ...
        for(var i=0;i<_Series.length;i++){
            for(var j=0;j<_Series[i].data.length;j++){
                Agi.Controls.CustomUChartAlarmStatisticalByData(j,_Series[i].data[j].y,AlarmStatisticaldata,SumPointNum);//统计标准线所在比例信息
            }
        }
    }
...
Agi.Controls.CustomUChartAlarmStatisticalByData=function(_index,_PointValue,_AlarmStatisticaldata,SumPointNum){
    //标准线超出点计算
...
//统计信息显示
Agi.Controls.CustomUChartAlarmStatisticalPanelShow=function(_AlarmStatisticaldata,_ApendToPanel){
    if($("#CustomUChartAlarmPanel").length>0){
        $("#CustomUChartAlarmPanel").html("");
    }else{
        var CustomUChartAlarmPanelHTML=$("<div id='CustomUChartAlarmPanel' class='AlarmSty'></div>").appendTo($(_ApendToPanel));
        $("#CustomUChartAlarmPanel").css({"right":"8px","top":"8px"});
    }
    var SubItemHTML="";
    ...
    $("#CustomUChartAlarmPanel").append(SubItemHTML);
}
...
//2.图形选项
Agi.Controls.CustomUChartView_ChartSetMenu=function(_Panel,_Control){
    var Me=_Control;
    //3.1.初始化数据准备
    var ChartEntity=Me.Get("Entity")[0];
    ...
    ItemContentPanel.load('js/Controls/CustomUChart/tabTemplates.html #tab-1', function () {
        $(this).find("#CustomUChartdataColor").spectrum({
            showInput: true,
            showPalette: true,
            palette: [
                ['black', 'white'],
        ...
        $(this).find("#CustomUChartmarkerColor").spectrum({
            showInput: true,
            showPalette: true,
            palette: [
                ['black', 'white'],
        ...
        //默认选中
        $(this).find("#CustomUChartdataColor").spectrum("set","#d2d2d2");
        $(this).find("#CustomUChartmarkerColor").spectrum("set","#4572A7");
        if(cp.series_DataColor!=null && cp.series_PointMarkerColor!=null){
            $(this).find("#CustomUChartdataColor").spectrum("set",cp.series_DataColor);
            $(this).find('#CustomUChartmarkerColor').spectrum("set",cp.series_PointMarkerColor);
        }
        //分组列和数据列绑定
        var GroupColumns=$(this).find('#CustomUChartGrpColumn');
        var DataColumns=$(this).find('#CustomUChartDataColumn');
        ...
        if(cp.SeriesGroupColumn && cp.SeriesDataColumn!=null){
            ...
            if(cp.SeriesGroupColumn==null){
                cp.SeriesGroupColumn=$($("#CustomUChartGrpColumn").children()[0]).val();
            }
            if(cp.SeriesDataColumn==null){
                cp.SeriesDataColumn=$($("#CustomUChartDataColumn").children()[0]).val();
            }
        }
        ...
        var CustNPChartSeriesItems= $(this).find("#CustomUChartSeries").empty();
        ...
        //保存处理
        $(this).find("#CustomUChartSeriesSave").unbind().bind("click",function(ev){
            var CustNPChartSeriesObj={
                SeriesName:$("#CustomUChartSeries").val(),
                GroupColumn:$("#CustomUChartGrpColumn").val(),
                DataColumn:$("#CustomUChartDataColumn").val(),
                SeriesColor:$("#CustomUChartdataColor").val(),
                SeriesMarkerColor:$("#CustomUChartmarkerColor").val(),
                GroupSize:{
                    Type:0,//类型
...
Agi.Controls.CustomUChartView_StandLinesMenu=function(_Panel,_Control){
    var Me=_Control;
    //3.1.初始化数据准备
    var ChartEntity=Me.Get("Entity")[0];
    var cp=Me.Get("ChartProperty");//获取属性
    ...
    //3.2.加载标准线设置菜单
    ItemContentPanel.load('js/Controls/CustomUChart/tabTemplates.html #tab-2', function () {
        //0.控件当前值获取
        ...
        $(this).find("#btnSaveGrp").click(function(){
            ...
            var line={
                GroupName:$("#CustomUChartStLinename").attr('value'),
...
Agi.Controls.CustomUChartView_SigmaMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('js/Controls/CustomUChart/tabTemplates.html #tab-3', function () {
        var THisChartSigmaRules=Me.Get("Sigmrules");//获取八项判异规则
        //1.八项规则保存
        $(this).find("#CustomUChartSigMaRuleSave").unbind().bind("click",function(ev){
            var rule=[];
            var ruleitem=null;
            for(i=1;i<=4;i++)
            {
...
//endreigon
//6.数据钻取设置
Agi.Controls.CustomUChartView_ExtractMenu=function(_Panel,_Control){
    var Me=_Control;
    var ChartEntity=Me.Get("Entity")[0];
    var ChartEntityColumns=[];
    if(ChartEntity!=null && ChartEntity.Columns!=null){
    ...
    ItemContentPanel.load('js/Controls/CustomUChart/tabTemplates.html #tab-4', function () {
        //0.默认选项隐藏
        $("#CustomUChartParCancel").hide();
        $("#CustomUChartNewParsTxt").width(0).hide();

        //1.添加参数
        $(this).find('#CustomUChartParAdd').click(function(){
            $(this).hide();
            $("#CustomUChartParCancel").show();
            $("#CustomUChartNewParsTxt").width(60).show();
            $("#CustomUChartParsNames").hide();
        });
        //2.取消添加
        $(this).find('#CustomUChartParCancel').click(function(){
            $("#CustomUChartParCancel").hide();
            $("#CustomUChartNewParsTxt").width(0).val("").hide();
            $("#CustomUChartParsNames").show();
            $("#CustomUChartParAdd").show();
        });
        //3.保存
        $(this).find('#CustomUChartParSave').click(function(){
            var SCSTxtVisibly=!$("#CustomUChartNewParsTxt").is(':hidden');
            var CusChartParsName="";
            var CusChartParsValue="";
            if(SCSTxtVisibly){
                //新增参数
                CusChartParsName=$("#CustomUChartNewParsTxt").val();
                CusChartParsValue=$("#CustomUChartParsValues").val();
            }else{
                //编辑参数
                CusChartParsName=$("#CustomUChartParsNames").val();
                CusChartParsValue=$("#CustomUChartParsValues").val();
            }
                ...
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    ...
                    if(SCSTxtVisibly){
                        if(ParIndex>-1){
                            ...
                            $("#CustomUChartParsNames").append("<option value='"+CusChartParsName+"'>"+CusChartParsName+"</option>");
                            ...
                            $("#CustomUChartParsNames").val(CusChartParsName);
                            //更新列表显示
                            Agi.Controls.CustomUChartParsListUpdate(ChartExtractConfig.Paras,"ParsListPanel");
        ...
        //4.删除
        $(this).find('#CustomUChartParDel').click(function(){
            var SCSTxtVisibly=!$("#CustomUChartNewParsTxt").is(':hidden');
            if(SCSTxtVisibly){
                AgiCommonDialogBox.Alert("请取消或完成新增，选择已存在的参数名称后再试!");
            }else{
                //编辑参数
               var CusChartParsName=$("#CustomUChartParsNames").val();
                if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
                    var ParIndex=-1;
                    for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                        if(ChartExtractConfig.Paras[i].Name==CusChartParsName){
                    ...
                    if(ParIndex>-1){
                        //移除对应的Option项
                        $("#CustomUChartParsNames option[value='"+ChartExtractConfig.Paras[ParIndex].Name+"']").remove(); //删除Select中Value=''的Option
        ...
        Agi.PageDataDrill.GetAllPageFilList(function(_result){
            if(_result.result && _result.result=="true"){
                ...
                if(_result.data!=null && _result.data.length>0){
                    ...
                    itemValue=null;
                    $("#CustomUChartExtractURL").html(strCustSigChartExtractURLoptions);
                    if(ChartExtractConfig!=null && ChartExtractConfig.URL!=null){
                        $("#CustomUChartExtractURL").find("[value='"+ChartExtractConfig.URL+"']").attr("selected","selected");
                    }
                }
            }
        })
        if(ChartExtractConfig.Paras!=null && ChartExtractConfig.Paras.length>0){
            for(var i=0;i<ChartExtractConfig.Paras.length;i++){
                $("#CustomUChartParsNames").append("<option value='"+ChartExtractConfig.Paras[i].Name+"'>"+ChartExtractConfig.Paras[i].Name+"</option>");
            }
            DefaultParNameColumn=ChartExtractConfig.Paras[0].Column;
            $("#CustomUChartParsNames").val(ChartExtractConfig.Paras[0].Name);

            //更新列表显示
            Agi.Controls.CustomUChartParsListUpdate(ChartExtractConfig.Paras,"ParsListPanel");
        }else{
            $("#CustomUChartParAdd").hide();
            $("#CustomUChartParCancel").show();
            $("#CustomUChartNewParsTxt").width(60).show();
            $("#CustomUChartParsNames").hide();
        }
        if(ChartEntityColumns!=null && ChartEntityColumns.length>0){
            for(var i=0;i<ChartEntityColumns.length;i++){
                $("#CustomUChartParsValues").append("<option value='"+ChartEntityColumns[i]+"'>"+ChartEntityColumns[i]+"</option>");
            }
            if(DefaultParNameColumn!=null){
                $("#CustomUChartParsValues").val(DefaultParNameColumn);
            }
        }
        //6.参数名称选中项更改
        $(this).find('#CustomUChartParsNames').change(function(){
            ...
            if(ThisParas!=null && ThisParas.length>0){
                for(var i=0;i<ThisParas.length;i++){
                    if(ThisParas[i].Name==StrThisSelValue){
                        $("#CustomUChartParsValues").val(ThisParas[i].Column);
                        break;
                    }
                }
            }
        ...
        $(this).find('#CstNPExtractOptionSave').click(function(){
            ...
            ThisExtractConfig.URL=$("#CustomUChartExtractURL").val();
...
//7.背景设置
Agi.Controls.CustomUChartView_BackgroundSetMenu=function(_Panel,_Control){
    var Me=_Control;
    //获取属性
    var cp = Me.Get('ChartProperty');
    ...
    ItemContentPanel.load('js/Controls/CustomUChart/tabTemplates.html #tab-5', function () {
        ...
        $("#CustomUChart_bgvalue").unbind().bind("click",function(){
        ...
        var ThisChartbgvalue=Agi.Controls.CustomUChartBackgroundValueGet(cp.ChartBackConfig.backgroundColor);
        $("#CustomUChart_bgvalue").css("background",ThisChartbgvalue.value.background).data('colorValue',ThisChartbgvalue);//设置默认项
        // backgroundColor//背景 ,Tolbtnbackground,//菜单按钮背景,TolbtnFontColor,//菜单按钮字体,TolTabbackground,//统计表格背景,TolTabFontColor//统计表格字体
        $("#CustomUChart_Tolbgvalue").spectrum("set",cp.ChartBackConfig.Tolbtnbackground);
        $("#CustomUChart_Tolbtncolor").spectrum("set",cp.ChartBackConfig.TolbtnFontColor);
        $("#CustomUChart_TolTabbg").spectrum("set",cp.ChartBackConfig.TolTabbackground);
        $("#CustomUChart_TolTabfontbg").spectrum("set",cp.ChartBackConfig.TolTabFontColor);

        $("#CustomUChart_BackgroundSave").unbind().bind("click",function(){
            var color=$("#CustomUChart_bgvalue").data('colorValue');
            ...
            Agi.Controls.CustomUChart.BackgroundApply(Me,BackgroudObj);//背景应用
            Me.RefreshByProPanelUp();//更新属性后，刷新图形显示
        })
    });
}
...
//region 编辑数据
//显示单值图数据钻取参数列表
Agi.Controls.CustomUChartParsListUpdate=function(_Paras,_ListPanelID){
    var StrListItems="";
    if(_Paras!=null && _Paras.length>0){
        for(var i=0;i<_Paras.length;i++){
            StrListItems+=" <div class='ParsListHeadSty'><div class='CustomUChartParsListCellSty'>"+_Paras[i].Name+"</div><div class='CustomUChartParsListCellSty'>"+_Paras[i].Column+"</div></div>";
        }
    }
...
//endregion
//region 背景设置
Agi.Controls.CustomUChartBackgroundValueGet=function(_oBackgroundObj){
...
Agi.Controls.CustomUChart.BackgroundApply=function(_ControlObj,_BackGroudObj){
...
Agi.Controls.CustomUChart.FindDataRowsByPoint=function(_checkDataSeries,_PointX){
//    var checkDataSeries = this.Get('CheckDataSeries');
    var PointRows=[];
...
Agi.Controls.CustomUChart.MinMaxLineData=function(_ThischeckSeriesObj,_MinMaxData,_IsMin,_linetype,_index){
...
//曲线Series 计算
Agi.Controls.CustomUChart.SeriesDataCal=function(_DataArray,_GroupValue){
    var TempDataArray=[];
