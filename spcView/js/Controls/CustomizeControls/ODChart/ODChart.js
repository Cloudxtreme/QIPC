...
Agi.Controls.ODChart = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
...
//                }
        ...
        RemoveEntity: function (_EntityKey) {
            if (!_EntityKey) {
                throw 'ODChart.RemoveEntity Arg is null';
            }
            var self = this;
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            self.AttributeList = [];
            self.Set("Entity", []);
            self.Set("ControlType", "ODChart");
            /* 20121121 markeluo SPC图修改  NO:201211211346 start */
            self.Set("Groupnrow",3);//默认分组大小，3个一组
            /*NO:201211211346 end */
            var ID = savedId ? savedId : "ODChart" + Agi.Script.CreateControlGUID();
        ...
        Copy: function () {
            if (layoutManagement.property.type == 1) {
                ...
                var NewDataGrid = new Agi.Controls.ODChart();
        ...
        UpdateChartSize: function () {
            ...
            if (self.chart) {
                ...
                //                    var h = self.shell.Body.height() - self.shell.Footer.height();
                var h = self.shell.Container.height(); // - self.shell.Title.height();
                //console.log(self.shell.BasicID +'- self.chart.setSize(w,h,false)  :'+w+','+h);
                if (w > 1 && h > 1) {
...
/*下拉列表控件参数更改处理方法*/
Agi.Controls.ODChartAttributeChange = function (ODChart, Key, _Value) {
    var self = ODChart;
    switch (Key) {
        case "Position":
        {
...
//拖拽控件至编辑页面时，初始控件的方法
Agi.Controls.InitODChart = function () {
    return new Agi.Controls.ODChart();
}

//ColumnChart 自定义属性面板初始化显示
Agi.Controls.ODChartProrityInit = function (ODChart) {
    var self = ODChart;
    ...
    basicPro.load('JS/Controls/CustomizeControls/ODChart/ODChartSetting.html #ODChartBasicSetting1', function () {
