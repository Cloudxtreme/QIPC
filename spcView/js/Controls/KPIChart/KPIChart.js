...
Agi.Controls.KPIChart = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
        ...
        Init: function (_Target, _ShowPosition) {
            ...
            var HTMLElementPanel = $("<div recivedata='true' id='Panel_" + ID + "' class='PanelSty KPIChartPanelSty'></div>");
        ...
        BindChart:function () {
            ...
            if(MePrority.CheckedIndex=="-1"){//-1表示重绘所有chart
                setTimeout(function(){
                    for(var i=0;i<KPIChartJson.length;i++){
                        var mychart=MePrority.ID+"myChart"+i;
                        if (FusionCharts(mychart)) {
                            FusionCharts(mychart).dispose();
                        }
                        var chart = new FusionCharts("JS/Controls/KPIChart/image/"+flashName, mychart, "100%", "100%", "0", "1");
            ...
            switch(Key) {
                ...
                case "ProPerty":
                    ...
                    if(_Value.IsReDrawChart){
                        if(_Value.CheckedIndex=="-1"){//-1表示属性的变化将会影响整体，图表需全部重绘
                            var length=parseInt(_Value.BasciObj.length);
                            //var height=_Value.Height/(length+1);
                            var ControlHTML="<ul>";
                    ...
                    HTMLElementPanel.find("ul").css(_Value.BgColor.value)
        ...
        CreateControl: function (_Config, _Target) {debugger;
            var Me = this;
            Me.YL=true;
            Me.IsCoordinate=true;
            Me.AttributeList = [];
            if (_Config != null) {
                if (_Target != null && _Target != "") {
                    ...
                    var HTMLElementPanel = $("<div recivedata='true' id='" + ID + "' class='PanelSty KPIChartPanelSty'></div>");
                    HTMLElementPanel.css('padding-bottom', '0px');
                    var PostionValue = { Left:0, Top:0, Right:0, Bottom:0 };
                    var obj = null;
                    if (typeof (_Target) == "string") {
...
Agi.Controls.KPIChartProrityInit = function (_KPIChart){
    ...
    ItemContent.append("<td class='KPIChartTabletd0'>控件标题：</td><td colspan='3' class='KPIChartTabletd1'>" +
        "<input id='KPIChart_Titletxt' type='text' style='width:60%;' >" +
    ...
    ItemContent.append("<td class='KPIChartTabletd0'>字体大小：</td>" +
        "<td class='KPIChartTabletd1'><input id='KPIChart_TitleFontSize' type='number' value='14' min='2' max='40'/></td>");
    ...
    ItemContent = $('<div class="BasicChart_Pro_Panel">' +
        ...
        '<tr>' +
        '<td class="prortityPanelTabletd0"><label for="KPIChart_DataTitleTxt">数据标题:</label></td><td colspan="3" class="prortityPanelTabletd1"><input id="KPIChart_DataTitleTxt" type="text" style="width:60%;" ><div id="KPIChart_DataTitleSave" class="KPIChartPropSavebuttonsty" title="保存"></div></td>' +
        '</tr>' +
        '<tr>' +
        '<td class="prortityPanelTabletd0"><label for="KPIChart_SymbolTxt">单位符号:</label></td><td colspan="3" class="prortityPanelTabletd1"><input id="KPIChart_SymbolTxt" type="text" style="width:60%;" ><div id="KPIChart_SymbolSave" class="KPIChartPropSavebuttonsty" title="保存"></div></td>' +
        '</tr>' +
        '<tr>' +
        ...
        '</tr>' +
        '<tr>' +
        '<td class="prortityPanelTabletd0">曲线类型：</td><td  class="prortityPanelTabletd1"><select data-field="SparkType" id="SparkType"><option value="SparkLine">线性图</option><option value="SparkColumn">条形图</option></select></td>' +
    ...
    $("#KPIChart_Delete").attr("disabled",true);
    $("#KPIChart_Delete").click(function(){
        for(var i=0;i<ThisProPerty.BasciObj.length;i++){
