        ...
        if (_Config != null) {
            if (_Target != null && _Target != "") {
                ...
                if (Entity.length > 0) {
                    ...
                    Agi.Utility.RequestData(EntityInfo, function (d) {
                        EntityInfo.Data = d;
                        Me.AddEntity(EntityInfo); /*添加实体*/
                    });
                      } else {
                         Me.AddEntity(EntityInfo); /*添加实体*/
                    }
...
Agi.Controls.LEDChartProrityInit = function (_LEDChart) {
   ...
   '<td class="prortityPanelTabletd0">数据列：</td><td class="prortityPanelTabletd1"><select data-field="ColumnName" id="ColumnName"></select></td>' +
    ...
    '<td class="prortityPanelTabletd0"></td><td   class="prortityPanelTabletd1"><input type="button" value="确定" id="btnLEDChartSave"/></td>' +
    '</tr>' +
    '<tr>' +
       '<td class="prortityPanelTabletd0">实时点位号：</td><td colspan="3" class="prortityPanelTabletd1"><input data-field="RealTimePoint" id="RealTimePoint" type="text" value=""/></td>' +
    '</tr>' +
    '<tr>' +
      '<td class="prortityPanelTabletd0">默认值：</td><td colspan="3" class="prortityPanelTabletd1"><input data-field="ledNum" id="ledNum" type="text" value=""/></td>' +
    '</tr>' +
    '<tr>' +
    ...
    '<td class="prortityPanelTabletd0">字体大小：</td><td  class="prortityPanelTabletd1"><input data-field="iDigitWidth" id="iDigitWidth" type ="number" value ="20" min="10" max = "100"/> </td>' +
    '</tr>' +
    //    '<tr>' +
    ...
    $('.prortityPanelTabletd1>input').change(function (obj) {
        ...
        switch (pName) {
            case "ledNum":
                var ledNum = $(this).val();
                if (!isNaN(ledNum)) {
                    LEDChartProperty.ledNum =Agi.Script.StringTrim(ledNum);
                }
                break;
            case "iDigitWidth":
    ...
    $("#btnLEDChartSave").click(function () {
        if (PointEditState) {
            Me.ReadOtherData(ThisProPerty.RealTimePoint);
    ...
    $("#ColumnName").change(function () {
        ...
        var ledNum = Entity[0].Data[0][cloumnName];
        if (!isNaN(ledNum)) {
