...
Agi.Controls.TimeSplider = Agi.OOP.Class.Create(Agi.Controls.ControlBasic,
    {
        ...
        Init: function (_Target, _ShowPosition, savedId) {/*控件初始化，_Target：显示到的容器，_ShowPosition：控件显示到的位置,left,top*/
            ...
            var BaseControlObj = $("<div id='" + ID + "' class='shadow'>" +
                                                       ...
                                                       "</div></div><div class='menucelltimesty' ><span id='SpanstartTime" + ID + "'></span>&nbsp; <span>--</span>&nbsp; <span id='SpanEndTime" + ID + "'>" +
                                                       "</span></div></div>");
...
//                //                $(".PointPanelsty").css("z-index", $(this).css("z-index")+1);
//                //                $(".StartPointsty").css("z-index", $(this).css("z-index")+2);
//                //                $(".EndPointsty").css("z-index", $(this).css("z-index")+2);
//            });
...
//                this.Set("BasicProperty", basicproperty);
//                GetNewSplider(this, basicproperty.ID, basicproperty.startDate, basicproperty.endDate, beginvalue, endvalue);
                this.Set("Position", _ThisPosition);
                PagePars = _ThisPosition = null;
            }
        },
...
Agi.Controls.TimeSpliderProrityInit = function (TimeSpliderControl) {
    ...
    ItemContent.append("<div>开始日期：<input type='date' id='begindate'></div>");
    ItemContent.append("<div>结束日期：<input  type='date' id='enddate'></div>");
    ItemContent.append("<div>选择开始值：<input type='date' id='beginvalue'></div>");
    ItemContent.append("<div>选择结束值：<input type='date' id='endvalue'></div>");
